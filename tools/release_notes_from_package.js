...
function buildReleaseNotesFromPackageManifest(input, options = {}) {
  ...
  return [
    ...
    '- File diff scope: full previous/current package inventory comparison by path, size, and SHA-256.',
    ...
    `- Manifest diff doc: \`${manifest.manifestDiffDoc || `docs/package-manifest-diff-v${manifest.version}.md`}\``,
    '',
    '## Handoff summary',
...
if (require.main === module) {
  ...
  if (!manifestPath) throw new Error('usage: node tools/release_notes_from_package.js --manifest=package-manifest-v<version>.json [--out=docs/release-notes-v<version>.md]');
