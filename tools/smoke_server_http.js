...
async function runFileopsSmoke(readerCookie, ownerAuthJsonHeaders) {
  ...
  novels = await loadNovels(readerCookie);
  const renamedSingle = findNovelByTitle(novels, 'renamed_single', rootCategory);
  ...
  novels = await loadNovels(readerCookie);
  const renamedSeries = findNovelByTitle(novels, 'series', rootCategory);
...
async function main() {
  ...
  try {
    ...
    assert.strictEqual(publicLoginHtml.headers['cache-control'], 'no-store', 'login html must remain no-store');
    assert.ok(publicLoginHtml.text.includes('rel="manifest" href="/manifest.json?v=rebuild-v564"'), 'login html must advertise manifest before auth');
    assert.ok(publicLoginHtml.text.includes('rel="apple-touch-icon" sizes="180x180" href="/icon/apple-icon-180x180.png"'), 'login html must advertise apple touch icon before auth');
    assert.ok(publicLoginHtml.text.includes('rel="icon" type="image/png" sizes="32x32" href="/icon/favicon-32x32.png"'), 'login html must advertise 32px favicon before auth');
    assert.ok(publicLoginHtml.text.includes('rel="icon" type="image/png" sizes="16x16" href="/icon/favicon-16x16.png"'), 'login html must advertise 16px favicon before auth');

    const publicManifest = await request(BASE + '/manifest.json?v=rebuild-v564', { headers: { 'accept-encoding': 'identity' } });
    ...
    assert.ok(String(faviconAlias.headers['cache-control'] || '').includes('immutable'), 'root favicon alias must be immutable cacheable');
    ...
    assert.strictEqual(readerSiteEntry.headers['cache-control'], 'no-store', 'reader site html must remain no-store');
    ...
    assert.strictEqual(readerMobileEntry.headers['cache-control'], 'no-store', 'reader mobile html must remain no-store');
    const versionedRebuildAsset = await request(BASE + '/scripts/rebuild/main.mjs?v=rebuild-v564', { headers: { cookie: cookie, 'accept-encoding': 'identity' } });
...
assert.strictEqual(sharedCss.body.shared.progress.lastRead.novelId, smokeNovelId, 'read progress preserved');
    ...
    const shellSource = fs.readFileSync(path.join(ROOT, 'public', 'fragments', 'app-shell.html'), 'utf8');
    ...
    assert.ok(searchSource.indexOf('nsearchOverlay') >= 0 && searchSource.indexOf('closeSearch(app)') >= 0, 'search overlay close handler exists');
    ...
    const styleSource = fs.readFileSync(path.join(ROOT, 'public', 'styles', 'app.css'), 'utf8');
