...
function loadFrontendCheckSources({ toolsDir, frontendCheckFile }) {
  ...
  const coreShellSource = read(path.join(publicRoot, 'fragments', 'app-shell.html'));
  const deferredUiSource = read(path.join(publicRoot, 'fragments', 'deferred-ui.html'));
  const shellSource = coreShellSource + '\n' + deferredUiSource;
  const coreAppCssSource = read(path.join(publicRoot, 'styles', 'app.css'));
  const deferredUiCssSource = read(path.join(publicRoot, 'styles', 'deferred-ui.css'));
  const appCssSource = coreAppCssSource + '\n' + deferredUiCssSource;
  const ownerCssSource = read(path.join(publicRoot, 'styles', 'owner.css'));
  const libraryHtmlSource = read(path.join(publicRoot, 'library.html'));
