...
assert.ok(!siteHtml.includes('admin-users.css') && !mobileHtml.includes('admin-users.css'), 'reader shells must not load admin-users.css');
assert.ok(!siteHtml.includes('owner.css') && !mobileHtml.includes('owner.css'), 'reader shells must not eagerly load owner.css');
...
assert.ok(appCss.includes('#devdbg-modal-overlay,#devdbg-modal,#recovery-center-overlay,#recovery-center-modal{display:none}'), 'app.css must hide owner tool DOM before lazy owner.css');
['#content', '#novel-list', '.reader', '#safe-area-bar', '#nav-slider', '#nsearch-panel'].forEach(selector => {
  assert.ok(appCss.includes(selector), `app.css must retain reader/search shell selector ${selector}`);
});
['.user-workbench-tabs', '.user-create-card', '.user-list-card', '.user-edit-card', '.operation-grid'].forEach(selector => {
  ...
  assert.ok(!appCss.includes(selector), `app.css must not absorb admin selector ${selector}`);
});
['#devdbg-modal{display:none;position:fixed', '#recovery-center-modal{display:none;position:fixed', '.recovery-cache-management-panel .recovery-cache-controls'].forEach(selector => {
  ...
  assert.ok(!appCss.includes(selector), `app.css must not own rich owner selector ${selector}`);
});
