...
function runSearchCacheManifestSmoke(projectRoot = process.cwd()) {
  ...
  const loader = read(projectRoot, 'public/scripts/rebuild/features/library-catalog-loader.mjs');
  const runner = read(projectRoot, 'tools/run_smoke_tests.js');
  const docs = read(projectRoot, 'docs/performance-cache.md');

  assert.ok(cache.includes("const DB_VERSION = 2"), 'reader cache DB version must be upgraded for search manifest store');
