...
async function runDynamicMatcherSmoke() {
  ...
  const app = {
    ...
    api: { async content({ episodeId, chunk }, options = {}) {
      ...
      activeRequests += 1; maxActiveRequests = Math.max(maxActiveRequests, activeRequests); await delay(2); activeRequests -= 1; return payloads[`${episodeId}:${chunk}`];
    } },
    state: {
      current,
  ...
  assert.ok(maxActiveRequests > 1 && maxActiveRequests <= 4, 'multi-episode full scan must use bounded adaptive request concurrency');
  assert.strictEqual(app.state.search.stats.multiEpisodeTargetConcurrency, 2, 'high-tier fixture must expose bounded two-target concurrency');
  ...
  assert.strictEqual(app.state.search.stats.searchServerLoadMitigationPass, 'v549-search-auto-server-profile-pass', 'stats must expose current server-load profile marker');
}
...
(async () => {
  ...
  assert.ok(matcher.includes('SEARCH_MULTI_EPISODE_TARGET_CONCURRENCY_MAX = 2'), 'multi-episode search concurrency must remain bounded at 2');
  assert.ok(matcher.includes('v509-search-multi-episode-adaptive-concurrency-pass'), 'multi-episode search must expose adaptive concurrency marker');
