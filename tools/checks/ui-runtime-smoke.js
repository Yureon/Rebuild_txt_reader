...
async function runUiRuntimeSmoke(projectRoot) {
  ...
  const script = String.raw`
    ...
    const tabs = ['general-panel', 'data-panel', 'viewer-panel', 'func-panel'].map(id => { const tab = makeEl('button'); tab.dataset.tab = id; return tab; });
    const panels = ['general-panel', 'data-panel', 'viewer-panel', 'func-panel'].map(id => makeEl('div', id));
