...
assert.ok(loginHtml.includes(`rel="manifest" href="/manifest.json?v=${CURRENT_REBUILD_VERSION}"`), 'login page must advertise manifest before login');
assert.ok(loginHtml.includes(`rel="apple-touch-icon" sizes="180x180" href="/icon/apple-icon-180x180.png?v=${CURRENT_REBUILD_VERSION}"`), 'login page must advertise apple touch icon before login');
assert.ok(loginHtml.includes(`rel="icon" type="image/png" sizes="32x32" href="/icon/favicon-32x32.png?v=${CURRENT_REBUILD_VERSION}"`), 'login page must advertise 32px favicon before login');
assert.ok(loginHtml.includes(`rel="icon" type="image/png" sizes="16x16" href="/icon/favicon-16x16.png?v=${CURRENT_REBUILD_VERSION}"`), 'login page must advertise 16px favicon before login');
