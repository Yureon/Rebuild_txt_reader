...
function runAdminUserStateSnapshotSmoke() {
  ...
  const before = manager.createSnapshotForUserId('reader-a', 'manual_test', { source: 'smoke' });
  assert.ok(before.snapshot && before.snapshot.id.endsWith('.json'), 'created snapshot id invalid');
  const reset = manager.resetStateForUserId('reader-a');
  ...
  const downloaded = manager.readSnapshotForUserId('reader-a', before.snapshot.id);
  assert.ok(downloaded.state && downloaded.snapshot.id === before.snapshot.id, 'snapshot download must return state');
  const restored = manager.restoreSnapshotForUserId('reader-a', before.snapshot.id);
  ...
  return { pass: 'v401-admin-user-state-snapshot-smoke-pass', snapshotCount: manager.listSnapshotsForUserId('reader-a').snapshots.length };
}

if (require.main === module) console.log(JSON.stringify(runAdminUserStateSnapshotSmoke()));
