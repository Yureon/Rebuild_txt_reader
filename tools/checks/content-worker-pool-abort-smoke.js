...
async function run(projectRoot = path.resolve(__dirname, '..', '..')) {
  ...
  assert.ok(envSource.includes('CONTENT_WORKER_THREADS_ENABLED') && envSource.includes('CONTENT_WORKER_POOL_SIZE'), 'env must expose worker-pool controls');
  assert.ok(appSource.includes('workerThreadsEnabled: CONTENT_WORKER_THREADS_ENABLED') && appSource.includes('workerPoolSize: CONTENT_WORKER_POOL_SIZE'), 'app must wire worker-pool env to content service');
  assert.ok(envExample.includes('CONTENT_WORKER_THREADS_ENABLED=1') && envExample.includes('CONTENT_WORKER_POOL_SIZE=0'), '.env.example must document worker-pool controls');
