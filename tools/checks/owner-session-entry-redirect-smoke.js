...
assert.ok(auth.includes("READER_ENTRY_PATHS = new Set(['/', '/index.html', '/library.html', '/site.html', '/mobile.html'])"), 'reader entry path allowlist missing');
...
assert.ok(serverSmoke.includes("owner library entry redirect status"), 'server HTTP smoke must assert owner /library.html redirect');
...
assert.ok(serverSmoke.includes("reader library entry status"), 'server HTTP smoke must assert reader /library.html works');
