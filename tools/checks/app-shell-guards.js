...
function runAppShellGuardChecks(ctx) {
  ...
  ['id="settings-panel-body" class="sp-body" data-settings-main-body="tabs" data-settings-tab-panels-owner="general viewer func data debug" data-settings-containment-pass="v231"','data-settings-panel-level="sp-body-child"','data-settings-tab-body="general"','data-settings-tab-body="viewer"','data-settings-tab-body="func"','data-settings-tab-body="data"','data-settings-tab-body="debug"','class="sp-tab-body"'].forEach((marker) => {
  ...
  const debugPanelIndex = shellSource.indexOf('id="debug-panel"');
  const viewerPanelIndex = shellSource.indexOf('id="viewer-panel"');
  const funcPanelIndex = shellSource.indexOf('id="func-panel"');
  const dataPanelIndex = shellSource.indexOf('id="data-panel"');
  const settingsCloseIndex = shellSource.indexOf('</section>', settingsBodyIndex);
  if (!(settingsBodyIndex >= 0 && settingsBodyIndex < generalPanelIndex && generalPanelIndex < dataPanelIndex && dataPanelIndex < debugPanelIndex && debugPanelIndex < viewerPanelIndex && viewerPanelIndex < funcPanelIndex && funcPanelIndex < settingsCloseIndex)) {
    throw new Error('Settings sp-body must contain general/data/debug/viewer/func tab panels as ordered descendants after v230');
  }
  const settingsPanelChains = getElementParentChainForIds(shellSource, ['general-panel', 'data-panel', 'debug-panel', 'viewer-panel', 'func-panel']);
  ['general-panel', 'data-panel', 'debug-panel', 'viewer-panel', 'func-panel'].forEach((id) => {
