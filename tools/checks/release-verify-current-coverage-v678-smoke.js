...
const currentRegressions=[
  'release-verify-current-coverage-v678-smoke.js',
  'v678-active-check-registration-smoke.js',
  'v678-extension-popup-fixed-width-smoke.js',
  'v678-metadata-folder-theme-smoke.js'
];
...
assert.equal(current.CURRENT_REBUILD_VERSION_NUMBER,678);
assert.equal(current.CURRENT_REBUILD_VERSION,'rebuild-v679');
assert.equal(current.CURRENT_REBUILD_PACKAGE,'txt_reader_v678.zip');
assert.equal(current.CURRENT_REBUILD_MANIFEST,'package-manifest-v678.json');
assert.equal(current.CURRENT_REBUILD_RELEASE_NOTES,'txt_reader_v678_change_report.md');
assert.equal(current.CURRENT_REBUILD_MANIFEST_DIFF,'package-manifest-diff-v678.md');
...
assert(read('tools/sync_version_contract.js').includes('v678-central-version-contract-pass'));
...
console.log(JSON.stringify({pass:'v678-release-verify-current-coverage-pass',currentRegressions:currentRegressions.length,build:'rebuild-v679'}));
