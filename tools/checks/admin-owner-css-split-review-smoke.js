...
assert.ok(deferredCss.includes('#devdbg-modal-overlay,#devdbg-modal,#recovery-center-overlay,#recovery-center-modal{display:none}'), 'deferred-ui.css must hide deferred owner tool DOM before owner.css lazy-loads');
...
assert.ok(!appCss.includes('#recovery-center-modal{display:none;position:fixed') && !appCss.includes('#devdbg-modal{display:none;position:fixed') && !deferredCss.includes('#recovery-center-modal{display:none;position:fixed') && !deferredCss.includes('#devdbg-modal{display:none;position:fixed'), 'core/deferred UI CSS must not keep rich owner modal positioning/layout rules');
assert.ok(!appCss.includes('.recovery-cache-management-panel .recovery-cache-controls') && !deferredCss.includes('.recovery-cache-management-panel .recovery-cache-controls'), 'core/deferred UI CSS must not keep recovery cache management layout');
['.recovery-chunk-overlay{position:fixed', '.recovery-log-box{', '.recovery-trial-filterbar{', '.recovery-action-button-row{', '#devdbg-modal[data-devdbg-quality-pass] .devdbg-output{'].forEach((selector) => {
  assert.ok(!appCss.includes(selector) && !deferredCss.includes(selector), `core/deferred UI CSS must not keep residual owner selector ${selector}`);
