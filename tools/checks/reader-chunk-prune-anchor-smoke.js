...
function runReaderChunkPruneAnchorSmoke(projectRoot) {
  ...
  [
    ...
    'restoreVirtualViewportAnchor(app, anchor'
  ].forEach(marker => {
  ...
  const rebuildIndex = chunkWindow.lastIndexOf("rebuildVirtualRows(app, 'prune'");
  const restoreIndex = chunkWindow.lastIndexOf('restoreVirtualViewportAnchor(app, anchor');
