...
assert.ok(source.includes('const CHUNK_PAYLOAD_CACHE_SCHEMA = 3'), 'chunk payload schema must be bumped');
...
assert.ok(source.includes('raw.normalizationVersion !== NORMALIZATION_ALGORITHM_VERSION'), 'read path must validate normalization version');
assert.ok(source.includes('raw.chunkIndexVersion !== CHUNK_INDEX_ALGORITHM_VERSION'), 'read path must validate chunk index version');
