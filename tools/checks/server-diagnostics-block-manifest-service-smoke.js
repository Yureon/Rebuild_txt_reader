...
const { invokeExpressJsonRoute, requireJsonRouteErrorResult, requireJsonRouteResult, requireWithMockedExpressRouter } = require('./server-smoke-fixtures.js');
...
async function runServerDiagnosticsBlockManifestServiceSmoke(projectRoot) {
  ...
  const contentService = {
    ...
    async getChunkByLineAsync(entry, chunk) {
  ...
  requireJsonRouteErrorResult(errorRouteRes, { statusCode:500, message:'internal_server_error', context:'single block-manifest error route' });
