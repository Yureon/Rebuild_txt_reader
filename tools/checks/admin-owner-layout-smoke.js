...
function runAdminOwnerLayoutSmoke() {
  ...
  assert.ok(page.includes(`/styles/admin-users.css?v=${CURRENT_REBUILD_VERSION}`), 'admin stylesheet cachebuster must match current rebuild');
  for (const rel of ['core','sections','ops','users','state','audit','signup','actions','audit-actions','signup-actions','state-actions','permissions']) {
    assert.ok(page.includes(`/scripts/admin/${rel}.js?v=${CURRENT_REBUILD_VERSION}`), rel + ' admin script cachebuster must match current rebuild');
  }
  assert.ok(page.includes(`/scripts/admin-users.js?v=${CURRENT_REBUILD_VERSION}`), 'admin main script cachebuster must match current rebuild');
