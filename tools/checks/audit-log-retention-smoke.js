...
async function runAuditLogRetentionSmoke() {
  ...
  try {
    ...
    await svc.flush();
    const status = await svc.getStatusAsync();
    ...
    const events = await svc.readEventsAsync({ limit: 5 });
...
if (require.main === module) runAuditLogRetentionSmoke().then(result => console.log(JSON.stringify(result))).catch(error => { console.error(error.stack || error); process.exitCode = 1; });
