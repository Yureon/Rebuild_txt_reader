...
async function runContentServiceSearchScanSmoke() {
  ...
  while (service.getCacheStatus().chunkPayloadWritesInflight > 0 && Date.now() - writeStartedAt < 3000) {
    await new Promise(resolve => setTimeout(resolve, 10));
  }
  assert.ok(listFiles(chunkPayloadDir).length > 0, 'normal reader chunk requests must still write chunk payload disk cache asynchronously');
