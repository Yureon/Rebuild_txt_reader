...
for (const file of ['release-verify-current-coverage-v631-smoke.js','service-worker-forward-upgrade-v631-smoke.js']) {
  assert(runner.includes(`tools/checks/${file}`), `quick runner missing v631 check: ${file}`);
  assert(verifier.includes(`tools/checks/${file}`), `release verifier missing v631 check: ${file}`);
}
assert(current.CURRENT_REBUILD_VERSION_NUMBER >= 631);
...
console.log(JSON.stringify({ pass:'v631-release-verify-current-coverage-pass', required:2 }));
