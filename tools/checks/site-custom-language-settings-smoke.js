...
function runSiteCustomLanguageSettingsSmoke() {
  ...
  assert(runtime.includes('CUSTOM_SITE_LANGUAGE_PREFIX'), 'custom site language prefix missing');
  assert(runtime.includes('normalizeSiteCustomLanguages'), 'custom language normalizer missing');
  ...
  assert(runtime.includes('renderSiteLanguageOptions'), 'language select custom option renderer missing');
