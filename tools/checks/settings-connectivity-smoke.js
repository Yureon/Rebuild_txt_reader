...
const dragDrop = fs.readFileSync('public/scripts/rebuild/features/library-drag-drop.mjs', 'utf8');
...
const dataStart = findOpeningDivById(html, 'data-panel');
const dataEnd = findMatchingDivClose(html, dataStart);
...
assert.ok(bodyStart < generalStart && generalEnd < dataStart && dataEnd < viewerStart && viewerEnd < funcStart && funcEnd < bodyEnd,
...
assert.ok(html.slice(dataStart, dataEnd).includes('data-settings-tab-body="data"'), 'data panel must contain data tab body');
assert.ok(html.slice(dataStart, dataEnd).includes('id="open-device-management-btn"'), 'data panel must own device management');
assert.ok(html.slice(dataStart, dataEnd).includes('id="settings-logout-btn"'), 'data panel must own account logout');
assert.ok(!html.slice(generalStart, generalEnd).includes('id="open-device-management-btn"'), 'general panel must not retain data management actions');
...
assert.ok(dragDrop.includes('LIBRARY_DND_HOVER_OPEN_DELAY_MS = 650'), 'library drag hover delay must be a fixed runtime constant');
