...
async function runLibraryMultifileDisclosureSmoke(projectRoot = path.resolve(__dirname, '..', '..')) {
  ...
  assert.strictEqual(eventMod.toggleEpisodeNovelRuntime(app, 'multi', deps), true, 'multi toggle should succeed');
  ...
  assert.strictEqual(eventMod.toggleEpisodeNovelRuntime(app, 'multi', deps), true, 'multi toggle collapse should succeed');
  assert.ok(!app.state.expandedEpisodeNovels.has('multi'), 'multi toggle should collapse');
  assert.strictEqual(eventMod.toggleEpisodeNovelRuntime(app, 'single', deps), false, 'single-file novel should not expose episode toggle behavior');
