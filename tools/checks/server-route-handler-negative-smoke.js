...
function createStateWriteService() {
  return {
    ...
    saveProgressDelta: () => ({ ok:true, route:'progress-delta' }),
  ...
  return {
    sendFsError: (res, error) => {
      ...
      return res.status(status).json(status >= 500
        ? { error:'internal_server_error', message:'library file operation failed' }
        : { error:error?.message || 'fs error' });
    },
    renameFolder: ok,
    renameNovel: ok,
    deleteNovel: ok,
...
function createFontService() {
  return {
    ...
    uploadFontAsync: async () => ({ success:true }),
    deleteFontAsync: async () => ({ success:true }),
...
function createFailingStateWriteService() {
  ...
  return {
    ...
    saveProgressDelta: () => fail('progress delta'),
...
function createFailingFontService() {
  ...
  return {
    ...
    uploadFontAsync: async () => fail('font upload'),
    deleteFontAsync: async () => fail('font delete'),
...
async function assertServiceFailure(router, routePath, method, reqOptions, context, expectedStatus = 500) {
  ...
  if (!res.jsonBody || res.jsonBody.error !== 'internal_server_error') throw new Error(context + ' expected masked service failure json error');
...
async function runServerRouteHandlerNegativeSmoke(projectRoot) {
  ...
  const passthroughCases = [
    ...
    [stateRouter, '/user-state/progress/:novelId', 'patch', { ...validRequest, params:{novelId:'n1'}, body:{ snapshot:{novelId:'n1',chunk:1} } }, 'state progress delta passthrough'],
  ...
  const serviceFailureCases = [
    ...
    [failingStateRouter, '/user-state/progress/:novelId', 'patch', { ...validRequest, params:{novelId:'n1'}, body:{ snapshot:{novelId:'n1',chunk:1} } }, 'state progress delta service failure', 503],
    ...
    [failingFontRouter, '/fonts', 'get', {}, 'font list service failure', 500],
