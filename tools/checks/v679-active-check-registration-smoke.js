...
const current=fs.readdirSync(checksDir).filter(name=>/^v679-.*-smoke\.(?:js|mjs)$/u.test(name)||name==='release-verify-current-coverage-v679-smoke.js').sort();
...
console.log(JSON.stringify({pass:'v679-active-check-registration-pass',registered:current.length,retired:Object.keys(SUPERSEDED_SMOKE_CHECKS).length,files:current}));
