...
const { buildContentSecurityPolicy, CLOUDFLARE_INSIGHTS_INLINE_HASHES } = require('../../server/middleware/security');
...
assert.ok(read('public/index.html').includes(`/scripts/entry-router.js?v=${CURRENT_REBUILD_VERSION}`), 'index external router script is required');
assert.ok(read('public/login.html').includes(`/scripts/login.js?v=${CURRENT_REBUILD_VERSION}`), 'login external script is required');
assert.ok(read('public/admin/users.html').includes(`/scripts/admin-users.js?v=${CURRENT_REBUILD_VERSION}`), 'admin external script is required');
assert.ok(read('public/index.html').includes(`/styles/entry-router.css?v=${CURRENT_REBUILD_VERSION}`), 'index external css is required');
assert.ok(read('public/login.html').includes(`/styles/login.css?v=${CURRENT_REBUILD_VERSION}`), 'login external css is required');
assert.ok(read('public/admin/users.html').includes(`/styles/admin-users.css?v=${CURRENT_REBUILD_VERSION}`), 'admin external css is required');
...
for (const hash of CLOUDFLARE_INSIGHTS_INLINE_HASHES) assert.ok(optInCsp.includes(hash), 'Cloudflare injected inline loader hash missing');
