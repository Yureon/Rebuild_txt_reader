...
const current = fs.readdirSync(checksDir).filter(name => /^v677-.*-smoke\.(?:js|mjs)$/u.test(name) || name === 'release-verify-current-coverage-v677-smoke.js').sort();
...
assert(current.length >= 7, `unexpected current smoke count ${current.length}`);
...
console.log(JSON.stringify({ pass:'v677-active-check-registration-pass', registered:current.length, retired:Object.keys(SUPERSEDED_SMOKE_CHECKS).length, files:current }));
