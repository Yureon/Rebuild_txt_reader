...
for (const html of [library, site, mobile]) {
  for (const href of ['/scripts/rebuild/features/settings/appearance.mjs']) {
    assert.ok(!html.includes(`<link rel="modulepreload" href="${href}">`), `deferred runtime must not be preloaded: ${href}`);
  }
}
...
assert.ok(serverSmoke.includes("/scripts/rebuild/main.mjs?v=' + CURRENT_REBUILD_VERSION"), 'server HTTP smoke must check current versioned main module dynamically');
