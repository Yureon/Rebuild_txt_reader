...
const required = [
  'release-verify-current-coverage-v670-smoke.js',
...
for (const file of required.slice(0,3)) {
...
for (const file of required.slice(3)) assert(fs.existsSync(path.join(root, 'tools/checks', file)), `related regression missing ${file}`);
...
for (const [doc, marker] of [
  ...
  ['docs/release-history.md','v671-release-history-pass'],
...
console.log(JSON.stringify({ pass:'v670-release-verify-current-coverage-pass', required:required.length }));
