...
(async () => {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'metadata-playwright-v584-'));
  ...
  try {
    ...
    console.log(JSON.stringify({ pass:'v584-metadata-playwright-profile-smoke-pass', launches:trace.launches.length, browserFetches:trace.requests.length }));
  } finally {
