...
assert.ok(dockerfile.includes('LIBRARY_DEEP_SIGNATURE_CHECK_TTL_MS=30000'), 'Dockerfile must default library deep signature check TTL');
assert.ok(dockerfile.includes('CONTENT_DISK_CACHE_MIN_BYTES=1048576'), 'Dockerfile must default normalized content disk cache threshold');
for (const path of ['docker-compose.yml', 'docker-compose.example.yml']) {
  ...
  assert.ok(compose.includes('CONTENT_DISK_CACHE_MIN_BYTES'), `${path} must pass CONTENT_DISK_CACHE_MIN_BYTES`);
}
...
for (const needle of ['APP_ORIGIN', 'URL=', 'DEPLOYMENT_MODE', 'REQUIRE_STRICT_ORIGIN', '/healthz', 'OWNER_PASSWORD_MIN_LENGTH', '허용 하한은 10', 'USER_PASSWORD_MIN_LENGTH', 'ALLOW_CLOUDFLARE_INSIGHTS', 'ALLOW_BLOB_WORKER', 'LIBRARY_DEEP_SIGNATURE_CHECK_TTL_MS', 'CONTENT_WORKER_QUEUE_MAX', 'CONTENT_MAIN_THREAD_FALLBACK_MAX_BYTES', 'CONTENT_DISK_CACHE_MIN_BYTES']) assert.ok(env.includes(needle), `.env.example must mention ${needle}`);
