...
async function runAuditLogServiceSmoke() {
  ...
  assert.strictEqual(result.ok, true, 'audit append must queue');
  await svc.flush();
...
if (require.main === module) runAuditLogServiceSmoke().then(result => console.log(JSON.stringify(result))).catch(error => { console.error(error.stack || error); process.exitCode = 1; });
