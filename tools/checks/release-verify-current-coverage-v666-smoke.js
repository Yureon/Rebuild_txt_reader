...
const required = [
  'release-verify-current-coverage-v666-smoke.js',
  'v666-non-auth-autofill-guard-smoke.js',
  'release-verify-current-coverage-v665-smoke.js',
  'v665-manual-cover-recognition-smoke.js',
  'v665-node-direct-launch-smoke.js',
  'v665-docs-consolidation-smoke.js',
...
assert(guard.includes("var GUARD_VERSION = 'v667'"));
assert(guard.includes("'data-lpignore': 'true'"));
assert(guard.includes("'data-1p-ignore': 'true'"));
assert(guard.includes('new MutationObserver'));
assert(read('public/sw.js').includes('`/scripts/non-auth-autofill-guard.js?v=${BUILD}`'));
assert(read('server/middleware/auth.js').includes("'/scripts/non-auth-autofill-guard.js'"));
console.log(JSON.stringify({ pass:'v666-release-verify-current-coverage-pass', required:required.length }));
