...
function runServerRouteDirectSmoke(projectRoot) {
  ...
  ["router.put('/user-state/shared'", "router.put('/user-state/progress'", "router.patch('/user-state/progress/:novelId'", "router.put('/user-state/device'"].forEach(route => requireGuardedStateMutation(stateRoutes, route));
  ...
  requireRouteWindowMarkers(fontRoutes, "router.get('/fonts/file/:filename'", 1100, ['requireFontSession', 'Cache-Control', 'private, max-age=0, must-revalidate', 'Content-Disposition'], 'v426 scoped font file response route');
  ...
  requireAllMarkers(recoveryRoutes.slice(recoveryIndex, recoveryIndex + 520), ['setNoStore(res)', 'getRecoveryStatusAsync', 'res.json(status)'], 'v221 recovery status route no-store smoke');
