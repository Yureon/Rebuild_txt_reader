...
assert.ok(librarySource.includes('const signatureCheckDue = effectiveTtl === 0 || now - (libraryCache.lastDeepSignatureCheckAt || 0) >= effectiveTtl'), 'library hot path must apply the effective deep-signature TTL');
assert.ok(librarySource.includes("if (signatureCheckDue) scheduleLibrarySignatureCheck('request-cache-hit')"), 'library hot path must schedule bounded async signature inspection');
assert.ok(librarySource.includes('Signature I/O never runs on the request path'), 'library request path must serve the last known-good snapshot during async inspection');
...
try {
  ...
  if (error && error.code === 'MODULE_NOT_FOUND') {
    console.log(JSON.stringify({ pass: 'v439-cache-strategy-adjustment-smoke-pass', mode: 'static-only', staticOnly:true, blockedDependency:'runtime-dependencies', reason: 'dependencies-not-installed' }));
    process.exit(77);
  }
  throw error;
}
...
try {
  ...
  returnPromise(contentService.getCachedFileEntryAsync(contentFile, {})).then(async (entry) => {
    ...
    assert.strictEqual(status.metrics.statSignatureCalls, 0, 'async content load must not use synchronous stat signatures');
    assert.strictEqual(status.metrics.asyncStatSignatureCalls, 1, 'first async content load avoids duplicate async stat signatures');
    ...
    const manifestContentService = {
      ...
      getTotalChunks: () => 2,
      getChunkByLineAsync: async (entry, chunk) => ({ content: chunk === 1 ? '문단 하나입니다.' : '문단 둘입니다.', start: chunk === 1 ? 0 : 9, end: chunk === 1 ? 9 : 20 })
    };
