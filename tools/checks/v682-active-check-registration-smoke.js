...
const current=fs.readdirSync(checksDir).filter(name=>/^v682-.*-smoke\.(?:js|mjs)$/u.test(name)||name==='release-verify-current-coverage-v682-smoke.js').sort();
...
console.log(JSON.stringify({pass:'v682-active-check-registration-pass',registered:current.length,retired:Object.keys(SUPERSEDED_SMOKE_CHECKS).length,files:current}));
