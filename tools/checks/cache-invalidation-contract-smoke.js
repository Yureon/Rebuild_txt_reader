...
async function runDynamicChecks() {
  ...
  try {
    ...
    const first = await contentService.getCachedFileEntryAsync(file, {});
    assert.ok(first && first.text.includes('alpha'), 'first content load');
    const firstSig = first.statSig;
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 10);
    fs.appendFileSync(file, '\nbeta', 'utf8');
    const second = await contentService.getCachedFileEntryAsync(file, {});
...
runDynamicChecks().then(() => {
  ...
  if (error && error.code === 'MODULE_NOT_FOUND') {
    console.log(JSON.stringify({ pass: PASS, mode: 'static-only', staticOnly:true, blockedDependency:'runtime-dependencies', reason: 'dependencies-not-installed' }));
    process.exitCode = 77;
    return;
  }
