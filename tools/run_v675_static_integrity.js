...
const staticOutput = path.join(root, 'v675_full_tree_static_scan.json');
const integrityOutput = path.join(root, 'v675_static_integrity_results.json');
...
const scanRoots = ['server', 'public/scripts', 'public/styles', 'extensions', 'tools'];
const scanExtensions = new Set(['.js', '.mjs', '.cjs', '.css', '.html']);
const scanFiles = scanRoots.flatMap(item => walk(path.join(root, item), file => scanExtensions.has(path.extname(file).toLowerCase()))).sort();
const patterns = {
...
const staticPayload = {
  pass:'v675-full-tree-static-pattern-scan-pass',
...
const payload = {
  schemaVersion:1,
  pass:'v675-static-integrity-pass',
  version:675,
  packageVersion:'6.75.0',
  buildId:'rebuild-v675',
  generatedAt:new Date().toISOString(),
  javascriptSyntax:{
    pass:'v675-javascript-syntax-pass',
  ...
  fullTreeStaticScan:{ pass:staticPayload.pass, files:staticPayload.files, bytes:staticPayload.bytes }
};
...
if (syntaxFailures.length || frontendRun.status !== 0 || gzipFailures.length || brotliFailures.length) process.exitCode = 1;
