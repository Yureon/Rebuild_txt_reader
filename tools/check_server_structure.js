...
if (!serverJs.includes("./server/bootstrap")) {
...
runServerContentPreprocessSmoke(root);
...
runLibraryDeepSignatureCacheSmoke(root);
runPrecompressedStaticSmoke(root);
runDocsConsolidationSmoke(root);
runContentCacheRawTextSmoke(root);
...
(async () => {
  ...
  for (const token of ['COPY --chown=node:node server ./server', 'COPY --chown=node:node public ./public', 'COPY --chown=node:node tools ./tools']) {
  ...
  for (const token of ['build: .', 'PORT=3000', '${LIBRARY_PATH}:/library:ro', 'read_only: true', 'cap_drop:', 'no-new-privileges:true']) {
