...
function buildLatestDocIndex({ version, cacheVersion, docs = [] } = {}) {
  ...
  const base = [
    ...
    `- Package: \`txt_reader_rebuild_v${version}.zip\`.`
  ];
...
if (require.main === module) {
  ...
  const version = Number(options.version);
  if (!Number.isInteger(version)) throw new Error('usage: node tools/rebuild_doc_scaffold.js --version=251 [--cache=rebuild-v251] [--summary=a|b] [--candidate=a] [--progress=96~98%] [--dry-run] [--out=/project]');
