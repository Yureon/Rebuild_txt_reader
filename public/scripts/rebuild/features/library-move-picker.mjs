...
export function chooseLibraryMoveTarget(app, target, {
  ...
  return new Promise(resolve => {
    ...
    const overlay = createEl('div', { class:'library-move-picker-overlay open' });
    const titleId = `library-move-picker-title-${Date.now()}`;
    const panel = createEl('div', { class:'library-move-picker', role:'dialog', 'aria-modal':'true', 'aria-labelledby':titleId, tabindex:'-1' });
    const title = createEl('div', { id:titleId, class:'library-move-picker-title', text: `${targetTypeLabel(target.type)} 이동` });
    ...
    deactivateFocus = activateModalFocus({ overlay, dialog:panel, initialFocus:search, onRequestClose:() => close(null), accessibleName:`${targetTypeLabel(target.type)} 이동` });
