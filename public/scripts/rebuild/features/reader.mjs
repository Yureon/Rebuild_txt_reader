...
import { applyBlockManifest, applyFolderBlockManifest, folderRatioToEpisodeTarget, hasBlockManifest, hasFolderBlockManifest, resetCoordinateState, resolveGlobalBlock, ratioToChunkTarget } from './reader/coordinates.mjs';
...
import { buildReaderOpenCurrentState, resetReaderOpenRuntimeState } from './reader/open-state.mjs';
...
import { isReaderRequestCurrent, manifestRequestKey, resetReaderRequestScope as resetReaderRequestScopeCore } from './reader/request-guards.mjs';
...
export function installReader(app) {
  ...
  app.reader = {
    ...
    persistProgress: () => { persistProgress(app.state); saveProgress(app, { immediate:true }); },
...
async function openNovel(app, novel, opts = {}) {
  ...
  const manifestPromise = ensureBlockManifest(app, { signal: app.state.chunkFetchAbort?.signal });
  const folderManifestPromise = ensureFolderBlockManifest(app, { signal: app.state.chunkFetchAbort?.signal });
  ...
  await loadChunk(app, app.state.current.chunk, 'replace', buildReaderOpenNovelLoadIntent({
    ...opts,
    ratio: app.state.current.ratio
  }));
  warmAdjacentChunks(app, (chunk, mode, options) => loadChunk(app, chunk, mode, options), { centerChunk: app.state.current?.chunk, direction: 'forward' });
  rememberRecent(app);
...
async function loadChunk(app, chunk, mode = 'replace', options = {}) {
  ...
  if (app.state.loadingChunks.has(key)) return null;
  ...
  try {
    const data = await loadChunkPayload(app, c, targetChunk, signal);
...
async function ensureBlockManifest(app, options = {}) {
  ...
  }, { signal: options.signal }).then(manifest => {
    if (!app.state.current || manifestRequestKey(app, app.state.current) !== key) return null;
...
async function loadChunkPayload(app, current, targetChunk, signal) {
...
async function goSliderPosition(app, ratio = 0, meta = {}) {
  ...
  try {
    ...
    if (v.pendingSliderMeasureTarget) {
      ...
      window.setTimeout(() => updateProgressFromViewport(app), 80);
    } else {
      updateProgressFromViewport(app);
    }
  }
...
function restoreReaderViewportTransitionAnchor(app, reason = 'viewport-transition') {
  ...
  delays.forEach((delay, index) => {
    window.setTimeout(() => {
      ...
      if (index === delays.length - 1) v.pendingViewportTransitionAnchor = null;
    }, delay);
  });
  return true;
}
