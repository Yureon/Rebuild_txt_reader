...
import { buildReaderPageUrl } from './library-navigation-actions.mjs';
...
export function openLibraryQuickItem(app, item, deps = {}) {
  ...
  if (app?.profile === 'library') {
    const href = buildReaderPageUrl(novelId, episodeId);
    globalThis.window?.location?.assign?.(href);
    return { opened:true, navigated:true, href, novelId, episodeId:episodeId || null };
  }
  if (!app?.reader?.openNovel) return false;
  if (episodeId) {
...
export function revealLibraryQuickItemInList(app, item, deps = {}) {
  ...
  const categoryParts = getCategoryParts(novel);
  categoryParts.reduce((parts, part) => {
    parts.push(part);
  ...
  if (app.state.libraryViewMode === 'explorer') {
    app.state.libraryExplorerPath = categoryParts.join('>');
    persistLibraryUi(app.state);
    if (typeof deps.renderLibrary === 'function') deps.renderLibrary(app, { source:'quick-reveal-explorer', resetScroll:true, followActive:false });
    window.requestAnimationFrame(() => window.requestAnimationFrame(() => highlightLibraryQuickTarget(app, novelId, episodeId)));
    if (typeof deps.toast === 'function') deps.toast(app, 'info', '목록 위치', categoryParts.length ? '작품이 있는 폴더로 이동했습니다.' : '라이브러리 루트로 이동했습니다.');
    return true;
  }
  persistLibraryUi(app.state);
