...
export function renderLibraryVirtualIfEnabledRuntime(app, box, novels, options = {}, deps = {}) {
  if (!deps.isLibraryVirtualRendererEnabled?.(app)) return { rendered:false, reason:'flag-off' };
  try {
    ...
    return { rendered:false, reason:'render-exception' };
  }
}
...
export function renderLibraryVirtualWindowRuntime(app, box, novels, options = {}, deps = {}) {
  ...
  if (!gate.allowed) {
    ...
    return { rendered:false, reason:gate.reason || 'gate-blocked' };
  }
