...
export function installReadDataModal(app) {
  ...
  on(app.els.openReadDataBtn, 'click', () => {
...
export function renderReadData(app) {
  ...
  box.innerHTML = '';
  if (tab === 'bookmarks') return renderBookmarkData(app, box);
  if (tab === 'recents') return renderRecentData(app, box);
  if (tab === 'favorites') return renderFavoriteData(app, box);
  return renderProgressData(app, box);
}
...
function renderProgressData(app, box) {
  const snaps = getProgressSnapshots(app);
  if (!snaps.length) return appendEmpty(box, '저장된 읽기 위치가 없습니다.');
  const groups = groupProgressSnapshotsByNovel(app, snaps);
  groups.forEach(group => {
    const stale = !group.novel;
    const multi = group.isMultiEpisode && group.items.length > 1;
    if (!multi) {
      const snap = group.items[0];
      box.append(renderProgressSnapshotRow(app, snap, stale));
      return;
    }
    const latest = group.items[0];
    const details = createEl('details', { class:`rdm-item rdm-folder-item${stale ? ' is-stale' : ''}`, open:false }, [
      createEl('summary', { class:'rdm-folder-summary' }, [
        createEl('div', { class:'rdm-info' }, [
          createEl('div', { class:'rdm-name', text: group.title }),
          createEl('div', { class:'rdm-meta', text:`${group.items.length}개 화수 위치 · 최근 ${formatSnapshotPosition(latest)} · ${formatDate(latest.ts)}` })
        ]),
      ...
      createEl('div', { class:'rdm-folder-children' }, group.items.map(snap => renderProgressSnapshotRow(app, snap, stale, true)))
    ]);
    box.append(details);
  });
}

function groupProgressSnapshotsByNovel(app, snaps = []) {
  const map = new Map();
  snaps.forEach(snap => {
    const novelId = String(snap.novelId || '');
    const novel = app.state.novelById.get(novelId);
    const entry = map.get(novelId) || { novelId, novel, title: novel?.title || snap.title || novelId, isMultiEpisode: Array.isArray(novel?.episodes) && novel.episodes.length > 1, items: [] };
...
function renderProgressSnapshotRow(app, snap, stale = false, nested = false) {
  return createEl('div', { class: `rdm-item${nested ? ' rdm-child-item' : ''}${stale ? ' is-stale' : ''}` }, [
    createEl('div', { class:'rdm-info' }, [
      createEl('div', { class:'rdm-name', text: labelForSnapshot(app, snap) }),
      createEl('div', { class:'rdm-meta', text:`${formatSnapshotPosition(snap)} · ${formatDate(snap.ts)}` })
    ]),
    createEl('div', { class:'rdm-actions' }, [
...
function renderBookmarkData(app, box) {
  ...
  list.forEach(bm => {
    ...
    box.append(createEl('div', { class: `rdm-item${stale ? ' is-stale' : ''}` }, [
      createEl('div', { class:'rdm-info' }, [
        createEl('div', { class:'rdm-name', text: bm.title || novel?.title || bm.novelId }),
        createEl('div', { class:'rdm-meta', text:`${formatSnapshotPosition(bm)} · ${formatDate(bm.ts)}` }),
...
function renderRecentData(app, box) {
  ...
  groups.forEach(group => {
    ...
    box.append(createEl('div', { class: `rdm-item${multi ? ' rdm-folder-item' : ''}${stale ? ' is-stale' : ''}` }, [
      createEl('div', { class:'rdm-info' }, [
        createEl('div', { class:'rdm-name', text: multi ? (group.novel?.title || item.title || group.novelId) : (item.title || item.label || group.novel?.title || item.novelId) }),
        createEl('div', { class:'rdm-meta', text: meta })
      ]),
      createEl('div', { class:'rdm-actions' }, [
...
function renderFavoriteData(app, box) {
  ...
  ids.forEach(id => {
    ...
    box.append(createEl('div', { class: `rdm-item${stale ? ' is-stale' : ''}` }, [
      createEl('div', { class:'rdm-info' }, [
        createEl('div', { class:'rdm-name', text: novel?.title || id }),
        createEl('div', { class:'rdm-meta', text: stale ? '목록에서 찾을 수 없음' : '즐겨찾기' })
      ]),
      createEl('div', { class:'rdm-actions' }, [
