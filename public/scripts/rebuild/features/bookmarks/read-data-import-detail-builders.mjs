import { formatDate, labelForSnapshot } from './read-data-model.mjs';
...
export function buildProgressImportDetail(app, currentProgress, incomingProgress) {
  return buildMapImportDetail(app, progressSnapshotMap(currentProgress), progressSnapshotMap(incomingProgress), item => labelForSnapshot(app, item));
}

export function buildArrayImportDetail(app, currentList, incomingList, keyFn) {
  return buildMapImportDetail(app, arrayToKeyMap(currentList || [], keyFn), arrayToKeyMap(incomingList || [], keyFn), item => item.title || item.label || labelForSnapshot(app, item));
}
...
export function buildFavoriteImportDetail(app, currentFavorites, incomingFavorites) {
  ...
  return buildMapImportDetail(app, current, incoming, item => app.state.novelById?.get?.(item?.novelId)?.title || item?.novelId || item?.id || '항목');
}
...
export function detailToSummary(detail) {
  return {
    ...
    staleList: uniqueList((detail.stale || []).map(entry => entry.incoming?.novelId || entry.key.split(':')[0] || entry.key))
  };
}
...
function formatImportEntryLabel(app, item, key, labelFn) {
  ...
  return item?.title || item?.label || novel?.title || String(key || '항목');
}
...
export function formatLimitedList(list, max = 8) {
  ...
  const values = unique.slice(0, max);
