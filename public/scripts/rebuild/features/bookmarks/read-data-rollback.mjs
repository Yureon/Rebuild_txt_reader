...
export function exportReadDataRollbackPreview(app) {
  ...
  const payload = {
    ...
    novelUserTags: snapshot.novelUserTags,
    userTagRecords: snapshot.userTagRecords || createUserTagRecords(snapshot.userTags, snapshot.novelUserTags)
  };
...
export function renderReadDataRollbackPanel(app, box, rollback) {
  ...
  const counts = latest ? rollbackCounts(latest) : { progress: 0, bookmarks: 0, recents: 0, favorites: 0, userTags: 0 };
  box.append(
    ...
    createEl('div', { class: 'rdm-preview-help', text: `가져오기 적용 직전 데이터입니다. 읽기 위치 ${counts.progress} · 북마크 ${counts.bookmarks} · 최근 ${counts.recents} · 즐겨찾기 ${counts.favorites} · 사용자 태그 ${counts.userTags}` }),
  ...
  history.forEach((snapshot, index) => {
    ...
    list.append(createEl('div', { class: 'rdm-rollback-row' }, [
      createEl('div', { class: 'rdm-rollback-info' }, [
        ...
        createEl('div', { class: 'rdm-rollback-meta', text: `위치 ${counts.progress} · 북마크 ${counts.bookmarks} · 최근 ${counts.recents} · 즐겨찾기 ${counts.favorites} · 사용자 태그 ${counts.userTags}` })
      ]),
...
export function rollbackCounts(snapshot = {}) {
  return {
    ...
    userTags: normalizeUserTagRecords(snapshot).length
  };
}
...
export function loadReadDataRollbackHistory() {
  ...
  const add = item => {
    ...
    const signature = [normalized.createdAt, normalized.sourceFileName, counts.progress, counts.bookmarks, counts.recents, counts.favorites, counts.userTags].join('|');
...
export function saveReadDataRollbackSnapshot(snapshot) {
  ...
  history.forEach(item => {
    ...
    const signature = [item.createdAt, item.sourceFileName, counts.progress, counts.bookmarks, counts.recents, counts.favorites, counts.userTags].join('|');
    if (seen.has(signature)) return;
    seen.add(signature);
    deduped.push(item);
  });
...
export function normalizeRollbackSnapshot(data = {}) {
  return {
    ...
    userTags: userTagRecordsToState(normalizeUserTagRecords(data)).userTags,
    novelUserTags: userTagRecordsToState(normalizeUserTagRecords(data)).novelUserTags,
...
export function applyReadDataSnapshot(app, snapshot) {
  ...
  app.state.userTags = normalized.userTags;
  app.state.novelUserTags = normalized.novelUserTags;
...
export function persistReadDataState(app) {
  ...
  app.state.libraryShelfFacetUserTags = Array.isArray(app.state.userTags) ? app.state.userTags.slice() : [];
  const scrollAnchor = app.library?.captureScrollAnchor?.() || null;
  const refresh = app.state.libraryViewMode === 'shelf'
    ? app.library?.loadShelf?.({ reset:true, preserveScroll:true, source:'read-data-user-tags-updated' })
    : app.library?.loadFullCatalog?.({ force:true, resetScroll:false, scrollAnchor, source:'read-data-user-tags-updated' });
  if (refresh && typeof refresh.catch === 'function') refresh.catch(() => {});
  else app.library?.render?.();
}
