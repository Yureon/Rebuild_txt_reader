...
import { isOpaqueReadDataIdentifier, shortReadDataIdentifier } from './read-data-model.mjs';
import {
...
export function renderImportPreviewResultRow(app, preview, entry, { renderPreview } = {}) {
  ...
  const info = createEl('div', { class: 'rdm-preview-result-info' }, [
    ...
    createEl('div', { class: 'rdm-preview-result-key', text: displayImportIdentifier(entry.key) })
  ]);
...
function displayImportIdentifier(value) {
  const text = String(value || '').trim();
  if (!text) return '';
  return isOpaqueReadDataIdentifier(text) ? shortReadDataIdentifier(text) : text.slice(0, 120);
}
...
export function createImportOverrideSelect(preview, typeId, entry, onChange = null) {
  ...
  select.addEventListener('change', () => {
    if (!isSafeStateRecordKey(typeId) || !isSafeStateRecordKey(entry.key)) return;
...
export function buildImportEntryFacts(entry) {
  ...
  const facts = [];
  if (item.tag) facts.push(`#${item.tag}`);
  if (Array.isArray(item.novelIds)) facts.push(`${item.novelIds.length}개 작품`);
  if (entry.staleNovelIds?.length) facts.push(`목록 없음 ${entry.staleNovelIds.length}개`);
  if (item.novelId) facts.push(`작품 식별자 ${displayImportIdentifier(item.novelId)}`);
  if (item.episodeId) facts.push(`회차 식별자 ${displayImportIdentifier(item.episodeId)}`);
