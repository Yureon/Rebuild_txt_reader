...
import { syncProgressState } from '../reader/progress.mjs';
...
export async function retryRecoverySaves(app, options = {}) {
  try {
    ...
    if (app.api?.putShared) {
      const sharedResult = await putSharedPatch(app, {
      ...
      if (sharedResult?.synced === false) throw new Error(sharedResult.reason || 'shared 저장 재시도 실패');
    }
    if (app.api?.putProgress) {
      const progressResult = await syncProgressState(app);
      if (progressResult?.synced === false) throw new Error(progressResult.reason || '진행률 저장 재시도 실패');
    }
