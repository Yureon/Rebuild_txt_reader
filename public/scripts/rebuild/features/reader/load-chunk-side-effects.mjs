...
export function reportChunkLoadFailure(app, error, context = {}) {
  ...
  if (workerBusy) {
    ...
    app.state.lastContentWorkerBusy = { pass:CONTENT_WORKER_RETRY_UX_PASS, mode:String(context.mode || ''), chunk:Number(context.targetChunk) || 0, at:Date.now() };
    if (foreground) toast(app, 'warn', '본문 처리 대기', '서버의 대용량 TXT 처리 대기열이 사용 중입니다. 잠시 뒤 다시 시도해 주세요.');
    return foreground;
  }
