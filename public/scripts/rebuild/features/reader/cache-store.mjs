...
import { getStorageScope } from '../../core/storage.mjs';
...
const CACHE_SCHEMA = 'reader-chunk-v2';
...
export const READER_CACHE_USER_SCOPE_PASS = 'v613-reader-cache-user-scope-pass';
...
function getReaderCacheScope(app = null) {
  const snapshot = app?.state?.userAccessSnapshot || {};
  const userId = String(snapshot.userId || app?.state?.userId || getStorageScope() || 'anonymous');
  const accessVersion = String(snapshot.accessVersion || app?.state?.accessVersion || '1');
  return { userId, accessVersion };
}

function makeScopedChunkCacheKey(app, current, chunk, preprocess = {}) {
  const base = makeChunkCacheKey(current, chunk, preprocess);
  const { userId, accessVersion } = getReaderCacheScope(app);
  return {
    ...base,
    id:[CACHE_SCHEMA, userId, accessVersion, base.novelId, base.episodeId, base.chunk, base.preprocessSignature, hashString(base.contentIdentity)].join('::'),
    userId,
    accessVersion,
    scopePass:READER_CACHE_USER_SCOPE_PASS
  };
}

function recordMatchesUserScope(record, app = null) {
  const { userId } = getReaderCacheScope(app);
  return String(record?.userId || '') === userId;
}

function recordMatchesCurrentScope(record, app = null) {
  const { userId, accessVersion } = getReaderCacheScope(app);
  return String(record?.userId || '') === userId && String(record?.accessVersion || '1') === accessVersion;
}

function filterRecordsForCurrentUser(records, app = null, { currentAccessVersion = false } = {}) {
  const predicate = currentAccessVersion ? recordMatchesCurrentScope : recordMatchesUserScope;
  return (Array.isArray(records) ? records : []).filter(record => predicate(record, app));
}
...
export async function readChunkPayloadFromCache(app, current, chunk) {
  ...
  const key = makeScopedChunkCacheKey(app, current, chunk, app.state.prefs.preprocess);
  try {
...
export async function clearReaderCache(app = null) {
  if (!isReaderCacheAvailable()) return false;
  try {
    cancelScheduledPrune();
    const db = await openDb();
    const chunks = filterRecordsForCurrentUser(await idbGetAll(db, CHUNK_STORE), app);
    const manifests = (await idbGetAll(db, SEARCH_MANIFEST_STORE)).filter(record => recordMatchesUserScope(record, app));
    await Promise.all([
      ...chunks.map(record => idbDelete(db, CHUNK_STORE, record.id)),
      ...manifests.map(record => idbDelete(db, SEARCH_MANIFEST_STORE, record.id))
    ]);
    return true;
  } catch (error) {
    disableOnError(error);
...
export async function deleteReaderCacheChunks(app, current = app?.state?.current, chunks = [], options = {}) {
  ...
  try {
    cancelScheduledPrune();
    const db = await openDb();
    const records = filterRecordsForCurrentUser(await idbGetAll(db, CHUNK_STORE), app);
    let removed = 0;
    let bytes = 0;
...
export async function getReaderCacheStats(app = null) {
...
export async function deleteReaderCacheForNovel(app, novelId, options = {}) {
  ...
  try {
    ...
    const manifest = await deleteSearchCacheManifestsForNovelInDb(db, targetNovelId, { episodeId, preprocessSignature: signature }, app);
...
export async function purgeReaderCacheOutsideAllowedNovelIds(app, allowedNovelIds = [], options = {}) {
  if (!isReaderCacheAvailable()) return { available: false, removed: 0, bytes: 0, error: disabled ? 'IndexedDB cache disabled' : 'IndexedDB unavailable' };
  ...
  try {
    ...
    const manifest = await purgeSearchCacheManifestOutsideAllowedNovelIdsInDb(db, allowed, app);
...
export async function purgeSearchCacheManifestOutsideAllowedNovelIds(app, allowedNovelIds = []) {
  ...
  try {
    const db = await openDb();
    const result = await purgeSearchCacheManifestOutsideAllowedNovelIdsInDb(db, allowed, app);
...
export async function deleteSearchCacheManifestForNovel(app, novelId, options = {}) {
  ...
  try {
    const db = await openDb();
    const result = await deleteSearchCacheManifestsForNovelInDb(db, targetNovelId, options, app);
...
async function purgeSearchCacheManifestOutsideAllowedNovelIdsInDb(db, allowed, app = null) {
  const records = (await idbGetAll(db, SEARCH_MANIFEST_STORE)).filter(record => recordMatchesUserScope(record, app));
  let removed = 0;
  const affectedNovels = new Set();
  for (const record of records) {
    const novelId = String(record?.novelId || '').trim();
...
async function deleteSearchCacheManifestsForNovelInDb(db, novelId, options = {}, app = null) {
  const targetNovelId = String(novelId || '').trim();
  const episodeId = options.episodeId == null ? null : String(options.episodeId || 'single');
  const signature = options.preprocessSignature == null ? null : String(options.preprocessSignature || '');
  const records = (await idbGetAll(db, SEARCH_MANIFEST_STORE)).filter(record => recordMatchesUserScope(record, app));
...
async function pruneDb(db, { maxEntries = DEFAULT_MAX_ENTRIES, maxBytes = DEFAULT_MAX_BYTES } = {}) {
  const records = filterRecordsForCurrentUser(await idbGetAll(db, CHUNK_STORE), null);
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(CHUNK_STORE)) {
        ...
        chunks.createIndex('userId', 'userId', { unique: false });
        chunks.createIndex('accessVersion', 'accessVersion', { unique: false });
      }
      if (db.objectStoreNames.contains(CHUNK_STORE)) {
        const chunks = req.transaction.objectStore(CHUNK_STORE);
        if (!chunks.indexNames.contains('userId')) chunks.createIndex('userId', 'userId', { unique:false });
        if (!chunks.indexNames.contains('accessVersion')) chunks.createIndex('accessVersion', 'accessVersion', { unique:false });
      }
...
export function resetReaderCacheRuntime() {
  cancelScheduledPrune();
  if (dbPromise) Promise.resolve(dbPromise).then(db => { try { db?.close?.(); } catch {} }).catch(() => {});
  dbPromise = null;
  prunePromise = null;
  disabled = false;
}

export async function purgeLegacyUnscopedReaderCache() {
  if (!isReaderCacheAvailable()) return { removed:0, manifestsRemoved:0, unavailable:true };
  const db = await openDb();
  const chunks = await idbGetAll(db, CHUNK_STORE);
  const manifests = await idbGetAll(db, SEARCH_MANIFEST_STORE);
  const legacyChunks = chunks.filter(record => !record?.userId);
  const legacyManifests = manifests.filter(record => !record?.userId);
  await Promise.all([
    ...legacyChunks.map(record => idbDelete(db, CHUNK_STORE, record.id)),
    ...legacyManifests.map(record => idbDelete(db, SEARCH_MANIFEST_STORE, record.id))
  ]);
  return { removed:legacyChunks.length, manifestsRemoved:legacyManifests.length, pass:READER_CACHE_USER_SCOPE_PASS };
}
