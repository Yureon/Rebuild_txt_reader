...
export function installJumpPanel(app, { goBlock, goPercent, goEpisode } = {}) {
  ...
  const open = () => {
    ...
    percentInput.min = '0'; percentInput.max = '100'; percentInput.step = '0.1'; percentInput.value = (ratio * 100).toFixed(1);
  ...
  on(slider, 'input', ev => syncPercent((Number(ev.target.value) || 0) / 10));
  on(percentInput, 'input', ev => syncPercent(ev.target.value));
  ...
  on(go, 'click', async () => {
    ...
    const ratio = clamp((Number(percentInput.value) || 0) / 100, 0, 1);
    const selectedEpisodeId = getSelectedEpisodeId(app, episodeSelect);
    const currentEpisodeId = app.state.current?.episode?.id || '';
