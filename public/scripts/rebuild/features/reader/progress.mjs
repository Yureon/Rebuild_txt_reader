...
export function snapshotProgress(app) {
  ...
  const novelTitle = String(c.novel?.title || c.novel?.fileName || '').trim();
  const episodeTitle = String(c.episode?.title || c.episode?.fileName || c.title || '').trim();
  return {
    novelId: c.novel.id,
    episodeId: c.episode?.id || null,
    novelTitle,
    episodeTitle:c.episode ? episodeTitle : '',
    title:c.episode && novelTitle && episodeTitle && novelTitle !== episodeTitle ? `${novelTitle} · ${episodeTitle}` : (episodeTitle || novelTitle),
...
export const READER_PROGRESS_DELTA_SYNC_PASS = 'v612-reader-progress-delta-sync-pass';
export const READER_PROGRESS_DUPLICATE_COALESCE_PASS = 'v612-reader-progress-duplicate-coalesce-pass';
export const READER_PROGRESS_RETRY_POLICY_PASS = 'v612-reader-progress-retry-policy-pass';

function progressSnapshotSignature(snapshot) {
  if (!snapshot) return '';
  return [
    snapshot.novelId || '', snapshot.episodeId || 'single', snapshot.chunk || 1,
    Number(snapshot.ratio || 0).toFixed(4), snapshot.globalBlockIndex ?? '',
    snapshot.blockIndex ?? '', snapshot.charIndex ?? '', snapshot.fileCharIndex ?? ''
  ].join('|');
}
...
function progressRetryDelay(error, attempt) {
  const retryAfterMs = Math.max(0, Number(error?.retryAfterSeconds) || 0) * 1000;
  if (retryAfterMs) return Math.min(5000, retryAfterMs);
  return Math.min(1500, 120 * Math.pow(2, Math.max(0, attempt - 1)));
}

function recordProgressPersistenceWarning(app, message) {
  if (!app?.state) return;
  const now = Date.now();
  const previousAt = Number(app.state.progressPersistenceWarningAt) || 0;
  app.state.progressPersistenceWarning = {
    pass:'v612-progress-persistence-warning-pass',
    message:String(message || '독서 위치를 기기에 저장하지 못했습니다.'),
    at:now
  };
  if (now - previousAt < 15000) return;
  app.state.progressPersistenceWarningAt = now;
  if (typeof document === 'undefined') return;
  void import('../ui.mjs').then(({ toast }) => {
    toast(app, 'warn', '기기 저장 확인 필요', app.state.progressPersistenceWarning.message);
  }).catch(() => {});
}

function monitorProgressPersistence(app, result) {
  if (!result) return;
  if (result.persistencePromise) {
    Promise.resolve(result.persistencePromise).then(status => monitorProgressPersistence(app, status)).catch(error => {
      recordProgressPersistenceWarning(app, `대용량 독서 기록 저장 실패: ${error?.message || String(error)}`);
    });
    return;
  }
  if (result.localSaved === false) recordProgressPersistenceWarning(app, '브라우저 로컬 저장 공간에 독서 위치를 기록하지 못했습니다.');
  if (result.idbError) {
    recordProgressPersistenceWarning(app, `대용량 독서 기록 저장 실패: ${result.idbError?.message || String(result.idbError)}`);
    return;
  }
  if (!result.idbResult?.ok && result.localCompacted) {
    recordProgressPersistenceWarning(app, '대용량 독서 기록을 IndexedDB에 저장하지 못해 최근 위치만 보존됩니다.');
  }
}

export async function saveProgress(app, options = {}) {
  const snap = snapshotProgress(app);
  if (!snap) return { synced:false, reason:'no-current-reader', attempts:0 };
  const signature = progressSnapshotSignature(snap);
  const lifecycle = options.lifecycle === true || options.keepalive === true;
  const duplicateWindowMs = lifecycle ? 1800 : 900;
  const successful = app.state.lastProgressSyncedFingerprint || null;
  const pending = lifecycle ? app.state.progressLifecycleSyncFingerprint : app.state.progressPendingSyncFingerprint;
  if (pending?.signature === signature && pending.promise) return pending.promise;
  if (successful?.signature === signature && Date.now() - (Number(successful.at) || 0) < duplicateWindowMs) {
    return { synced:true, skipped:true, reason:'duplicate-successful-progress-snapshot', attempts:0, pass:READER_PROGRESS_DUPLICATE_COALESCE_PASS };
  }
  ...
  const persistence = persistProgress(app.state, {
    immediate:lifecycle || options.immediate === true,
    recentSnapshot:snap
  });
  app.state.lastProgressLocalFingerprint = { signature, at:Date.now(), pass:READER_PROGRESS_DUPLICATE_COALESCE_PASS };
  monitorProgressPersistence(app, persistence);

  return syncProgressState(app, { ...options, snapshot:snap, signature });
}

async function executeProgressSync(app, snapshot, options = {}) {
  const useDelta = !!(snapshot && typeof app.api?.patchProgressNovel === 'function');
  let lastError = null;
  let lastResponse = null;
  const maxAttempts = options.keepalive === true ? 1 : 3;
  let attemptsMade = 0;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    attemptsMade = attempt;
    const nextVersion = Math.max(0, Number(app.state.sharedVersion) || 0) + 1;
    try {
      const payload = useDelta
        ? { snapshot, updatedAt:Date.now(), syncVersion:nextVersion }
        : { progress:app.state.progress, updatedAt:Date.now(), syncVersion:nextVersion };
      const res = useDelta
        ? await app.api.patchProgressNovel(snapshot.novelId, payload, { keepalive:options.keepalive === true })
        : await app.api.putProgress(payload, { keepalive:options.keepalive === true });
      lastResponse = res || null;
      if (Number.isFinite(Number(res?.sharedVersion))) app.state.sharedVersion = Math.max(0, Number(res.sharedVersion) || 0);
      if (res?.syncPolicySummary) app.state.syncPolicySummary = res.syncPolicySummary;
      if (!res?.skipped) {
        if (res?.shared) app.state.shared = res.shared;
        app.state.lastProgressSync = {
          pass:useDelta ? READER_PROGRESS_DELTA_SYNC_PASS : 'legacy-full-progress-sync',
          novelId:snapshot?.novelId || '',
          attempts:attempt,
          lifecycle:options.keepalive === true,
          at:Date.now()
        };
        app.state.lastProgressSyncedFingerprint = {
          signature:options.signature || progressSnapshotSignature(snapshot),
          at:Date.now(),
          pass:READER_PROGRESS_DUPLICATE_COALESCE_PASS
        };
        return { synced:true, response:res, attempts:attempt, mode:useDelta ? 'delta' : 'full' };
      }
      if (res?.reason !== 'stale_shared_version' && res?.reason !== 'stale_shared_updatedAt') break;
    } catch (error) {
      lastError = error;
      if (attempt >= maxAttempts || !isRetryableProgressError(error)) break;
      await sleep(progressRetryDelay(error, attempt));
    }
  }
  return {
    synced:false,
    ...
    reason:lastResponse?.skipped ? 'stale-progress-retry-exhausted' : 'request-failed',
    attempts:attemptsMade,
    mode:useDelta ? 'delta' : 'full',
    pass:READER_PROGRESS_RETRY_POLICY_PASS
  };
}

export function syncProgressState(app, options = {}) {
  const snapshot = options.snapshot && options.snapshot.novelId ? { ...options.snapshot } : null;
  const signature = options.signature || progressSnapshotSignature(snapshot);
  const lifecycle = options.lifecycle === true || options.keepalive === true;

  if (lifecycle) {
    const request = executeProgressSync(app, snapshot, { ...options, keepalive:true, signature });
    app.state.progressLifecycleSyncFingerprint = { signature, promise:request, at:Date.now() };
    app.state.progressLifecycleSyncRequest = request;
    return request.finally(() => {
      if (app.state.progressLifecycleSyncRequest === request) app.state.progressLifecycleSyncRequest = null;
      if (app.state.progressLifecycleSyncFingerprint?.promise === request) app.state.progressLifecycleSyncFingerprint = null;
    });
  }

  const previous = app.state.progressSyncRequest || Promise.resolve();
  const request = Promise.resolve(previous).catch(() => null).then(() => executeProgressSync(app, snapshot, { ...options, signature }));
  app.state.progressPendingSyncFingerprint = { signature, promise:request, at:Date.now() };
  app.state.progressSyncRequest = request;
  return request.finally(() => {
    if (app.state.progressSyncRequest === request) app.state.progressSyncRequest = null;
    if (app.state.progressPendingSyncFingerprint?.promise === request) app.state.progressPendingSyncFingerprint = null;
  });
}
...
export function rememberRecent(app) {
  ...
  void import('../bookmarks/model.mjs').then(({ persistAndSync })=>persistAndSync(app));
}
