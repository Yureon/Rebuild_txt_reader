...
import { decorateRowsWithGlobalBlocks, chunkCharToFileChar, chunkStateToDocumentRatio, fileCharToChunkAddress, fileCharToDocumentRatio, hasBlockManifest, READER_FULL_FILE_CHAR_PROGRESS_PASS } from './coordinates.mjs';
...
import { READER_RENDER_WINDOW_ANCHOR_PASS, captureVirtualRenderWindowAnchor, syncVirtualSpacerHeights } from './virtual-render-stability.mjs';
...
export const READER_NATIVE_LARGE_CORRECTION_GUARD_PASS = 'v565-reader-native-large-correction-guard-pass';
export const READER_VIRTUAL_LAYOUT_CACHE_PASS = 'v565-reader-virtual-layout-cache-pass';
const VIRTUAL_NATIVE_LARGE_CORRECTION_SETTLE_MS = 1200;
const VIRTUAL_NATIVE_LARGE_CORRECTION_MIN_PX = 280;
const VIRTUAL_NATIVE_LARGE_CORRECTION_MAX_PX = 720;
const VIRTUAL_NATIVE_LARGE_CORRECTION_VIEWPORT_RATIO = 0.72;
...
export function ensureVirtualState(app) {
  if (!app.state.readerVirtual) {
    app.state.readerVirtual = {
      ...
      measureRaf: 0,
      layoutRevision: 0,
      layoutComputedRevision: -1,
      layoutCacheHits: 0,
      layoutRecalcCount: 0,
      layoutDirtyReason: 'initial-state',
      virtualLayoutCachePass: READER_VIRTUAL_LAYOUT_CACHE_PASS,
      nativeLargeCorrectionGuardPass: READER_NATIVE_LARGE_CORRECTION_GUARD_PASS,
      lastNativeLargeCorrectionGuard: null,
...
function markVirtualLayoutDirty(v, reason = 'layout-mutation') {
  if (!v) return 0;
  v.layoutRevision = Math.max(0, Number(v.layoutRevision) || 0) + 1;
  v.layoutDirtyReason = String(reason || 'layout-mutation');
  return v.layoutRevision;
}

function markVirtualLayoutSynchronized(v, reason = 'incremental-layout') {
  const revision = markVirtualLayoutDirty(v, reason);
  v.layoutComputedRevision = revision;
  return revision;
}
...
export function resetVirtualDocument(app, { clearMeasures = false } = {}) {
  ...
  markVirtualLayoutSynchronized(v, 'reset-virtual-document');
...
export function rebuildVirtualRows(app, mode = 'replace', focusedChunk = 0, options = {}) {
  ...
  markVirtualLayoutDirty(v, `rebuild-${mode}`);
...
function resolveNativeLargeCorrectionGuard(v, reader, anchor, context = {}) {
  const now = Date.now();
  const anchorType = String(context?.anchorType || 'viewport');
  const phase = String(context?.phase || context?.reason || 'anchor-restore');
  const explicit = context?.explicit === true || context?.preserveAnchor === true;
  const pending = !!v?.pendingScrollTarget || !!v?.pendingSliderMeasureTarget;
  const nativeSource = isUserScrollSource(v?.lastUserScrollSource);
  const activeUntil = Number(v?.userScrollActiveUntil) || 0;
  const recentNativeScroll = nativeSource && activeUntil > 0 && now <= activeUntil + VIRTUAL_NATIVE_LARGE_CORRECTION_SETTLE_MS;
  const guardedAnchorType = anchorType === 'measure' || anchorType === 'render-window';
  const delta = estimateVirtualAnchorDelta(reader, v?.rows || [], v?.prefix || [0], anchor);
  const deltaPx = Number(delta?.deltaPx) || 0;
  const absDeltaPx = Math.abs(deltaPx);
  const viewport = Math.max(0, Number(reader?.clientHeight) || 0);
  const thresholdPx = Math.max(
    VIRTUAL_NATIVE_LARGE_CORRECTION_MIN_PX,
    Math.min(VIRTUAL_NATIVE_LARGE_CORRECTION_MAX_PX, Math.round(viewport * VIRTUAL_NATIVE_LARGE_CORRECTION_VIEWPORT_RATIO))
  );
  const suppress = !explicit && !pending && guardedAnchorType && recentNativeScroll && absDeltaPx > thresholdPx;
  const result = {
    pass: READER_NATIVE_LARGE_CORRECTION_GUARD_PASS,
    suppress,
    reason: suppress
      ? 'large delayed layout correction suppressed after native scrolling'
      : explicit
        ? 'explicit anchor restore bypasses large-correction guard'
        : pending
          ? 'pending navigation target bypasses large-correction guard'
          : !guardedAnchorType
            ? 'anchor type is not a delayed layout correction'
            : !recentNativeScroll
              ? 'native scroll settle window expired'
              : 'correction is within continuity threshold',
    phase,
    anchorType,
    deltaPx: Math.round(deltaPx),
    absDeltaPx: Math.round(absDeltaPx),
  ...
  if (v) {
    v.nativeLargeCorrectionGuardPass = READER_NATIVE_LARGE_CORRECTION_GUARD_PASS;
    v.lastNativeLargeCorrectionGuard = result;
  }
  return result;
}
...
function applyVirtualScrollAnchorWithInertiaGuard(app, v, reader, anchor, context = {}) {
  ...
  const largeCorrectionGuard = resolveNativeLargeCorrectionGuard(v, reader, anchor, context);
  if (largeCorrectionGuard.suppress) {
    return {
      applied: false,
      reason: largeCorrectionGuard.reason,
      pass: READER_NATIVE_LARGE_CORRECTION_GUARD_PASS,
      suppressedBy: READER_NATIVE_LARGE_CORRECTION_GUARD_PASS,
      rowId: largeCorrectionGuard.rowId,
      rowIndex: largeCorrectionGuard.rowIndex,
      deltaPx: largeCorrectionGuard.deltaPx
    };
  }
...
function resolveUnifiedAppendRestorePolicy(app, v, reader, anchor = null, context = {}, now = Date.now()) {
  ...
  const suppress = !!(reader && multiFile && !explicit && !pending && userScrollSource && direction !== 'backward' && (nativeForwardRetain.retain || seamTransitLock.locked || (append.recent && active)));
  const result = {
...
function appendVirtualRowsDelta(app, rowsToAppend = [], context = {}) {
  ...
  v.totalHeight = total;
  markVirtualLayoutSynchronized(v, 'append-virtual-rows-delta');
...
function tryPruneVirtualRowsIncrementally(app, mode, focusedChunk, context = {}) {
  ...
  rebuildVirtualRowIndexes(v);
  markVirtualLayoutSynchronized(v, 'prune-prefix-delta');
  renderVirtual(app, { force: true });
...
export function recalcVirtualLayout(app) {
  const v = ensureVirtualState(app);
  const revision = Math.max(0, Number(v.layoutRevision) || 0);
  const cacheValid = Number(v.layoutComputedRevision) === revision
    && Array.isArray(v.heights) && v.heights.length === v.rows.length
    && Array.isArray(v.prefix) && v.prefix.length === v.rows.length + 1
    && Number.isFinite(Number(v.totalHeight));
  if (cacheValid) {
    v.layoutCacheHits = Math.max(0, Number(v.layoutCacheHits) || 0) + 1;
    return v;
  }
  ...
  v.totalHeight = total;
  v.layoutComputedRevision = revision;
  v.layoutRecalcCount = Math.max(0, Number(v.layoutRecalcCount) || 0) + 1;
  v.virtualLayoutCachePass = READER_VIRTUAL_LAYOUT_CACHE_PASS;
  return v;
}
...
function resolveNativeForwardMeasureCommitFreeze(app = null, v = null, context = {}, now = Date.now()) {
  ...
  const freeze = !!(
    ...
    userScrollSource &&
    forward &&
    (append.recent || (appendWindow && (active || recentNativeSettle)) || recentAppendAnchor) &&
    (active || recentNativeSettle || recentAppendAnchor || settleExactRestore)
  );
...
function flushRenderedMeasureCacheForSettle(app, v, content, context = {}) {
  ...
  if (updates instanceof Map && updates.size) {
    ...
    if (changed) {
      markVirtualLayoutDirty(v, 'settled-measure-flush');
      recalcVirtualLayout(app);
    }
  }
  const result = {
...
function commitVirtualMeasureUpdates(app, updates) {
  ...
  markVirtualLayoutDirty(v, 'measure-commit');
  recalcVirtualLayout(app);
...
export function invalidateVirtualLayout(app) {
  ...
  v.renderedStart = -1;
  v.renderedEnd = -1;
  markVirtualLayoutDirty(v, 'invalidate-virtual-layout');
  recalcVirtualLayout(app);
  scheduleVirtualRender(app);
}
