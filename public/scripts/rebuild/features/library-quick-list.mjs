...
const LIBRARY_QUICK_UI_STORAGE_KEY = 'libraryQuickUi';
...
export function renderLibraryQuickList(app) {
  ...
  const recentSummary = filterActive ? emptyLibraryQuickSummary() : getLibraryQuickRecentItems(app, LIBRARY_QUICK_EXPANDED_LIMIT);
  const favoriteSummary = filterActive ? emptyLibraryQuickSummary() : getLibraryQuickFavoriteItems(app, LIBRARY_QUICK_EXPANDED_LIMIT);
  if (filterActive || (!recentSummary.total && !favoriteSummary.total && !recentSummary.stale && !favoriteSummary.stale)) {
    box.replaceChildren();
    box.hidden = true;
    return;
  }
  ...
  fragment.append(createLibraryQuickSection('recents', '최근 항목', recentSummary.items, recentSummary.total, ui, { staleCount: recentSummary.stale }));
  fragment.append(createLibraryQuickSection('favorites', '즐겨찾기', favoriteSummary.items, favoriteSummary.total, ui, { staleCount: favoriteSummary.stale }));
...
function getLibraryQuickUiState(app) {
  ...
  const fallback = { collapsed:{ recents:false, favorites:false }, expanded:{ recents:false, favorites:false } };
  try {
    const raw = window.localStorage?.getItem?.(LIBRARY_QUICK_UI_STORAGE_KEY);
    if (raw) {
      const parsed = JSON.parse(raw);
      app.state.libraryQuickUi = {
...
function createLibraryQuickSection(sectionId, label, items, totalCount, ui, options = {}) {
  ...
  const section = createEl('section', { class:`library-quick-section${collapsed ? ' is-collapsed' : ''}`, dataset:{ libraryQuickSection:key, libraryQuickStaleCount:String(staleCount) } }, [
    createEl('div', { class:'library-quick-head' }, [
      createEl('button', {
        ...
        createEl('span', { class:'library-quick-head-icon', text: key === 'favorites' ? '★' : '↺' }),
        createEl('span', { class:'library-quick-head-label', text:label })
      ]),
      createEl('span', { class:'library-quick-count', text:`${list.length}/${total}` })
    ])
  ]);
  if (collapsed) {
    const staleText = staleCount ? ` · 삭제/누락 ${staleCount}개 제외` : '';
    section.append(createEl('button', {
      class:'library-quick-collapsed-summary',
      type:'button',
      dataset:{ libraryQuickAction:'section-toggle', libraryQuickSection:key },
      text: total ? `${total}개 항목 숨김${staleText}` : `표시 항목 없음${staleText}`
    }));
    return section;
  }
  if (!list.length) {
  ...
  if (hiddenCount > 0 || (expanded && total > LIBRARY_QUICK_COLLAPSED_LIMIT)) {
    section.append(createEl('button', {
      ...
      text: expanded ? '접기' : `더보기 ${hiddenCount}개`
    }));
  }
  return section;
}
