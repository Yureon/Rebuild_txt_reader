...
import { CUSTOM_CSS_SCOPE_LABELS, buildCustomCssEditorState, buildCustomCssText, normalizeCustomCss, sanitizeCustomCss } from './custom-css-utils.mjs';

const STYLE_ID = 'txt-reader-user-css';
const CUSTOM_CSS_QUALITY_PASS = 'v314';
...
function getStyleEl() {
  ...
  if (!el) {
    ...
    el.id = STYLE_ID;
    el.dataset.source = 'txt-reader-user-css';
...
async function syncSharedCustomCss(app, css) {
  ...
  const shared = {
    ...
    viewerPrefs: {
      ...(current.viewerPrefs || {}),
      customCssShared: normalizeCustomCss(css)
    },
    updatedAt: Date.now()
  };
