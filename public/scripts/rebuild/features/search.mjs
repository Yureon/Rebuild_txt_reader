...
import { MAX_RESULTS, buildSearchCompletionSummary, buildSearchCoveragePreview, loadSearchServerPerformanceProfile, prewarmSearchWorker, searchAllChunks, searchSelectedChunks } from './search/matcher.mjs';
...
const SEARCH_ADAPTIVE_INPUT_CLEANUP_PASS = 'v591-search-adaptive-input-cleanup-pass';
...
export function installSearch(app) {
  ...
  app.state.search.adaptiveInputCleanupPass = SEARCH_ADAPTIVE_INPUT_CLEANUP_PASS;
  app.search = {
    open: () => openSearch(app),
  ...
  app.searchCleanup = () => {
    ...
    app.state.search.adaptiveInputCleanup?.();
    app.state.search.adaptiveInputCleanupPass = SEARCH_ADAPTIVE_INPUT_CLEANUP_PASS;
...
function closeSearch(app, options = {}) {
  ...
  if (abort) {
    ...
    app.state.search.running = false;
    app.state.search.resultCallbacks = null;
    setButtonBusy(app.els.nsearchRun, false);
  }
...
function syncSearchRunButton(app) {
  const button = app?.els?.nsearchRun;
  if (!button) return;
  const hasQuery = !!String(app.els?.nsearchInput?.value || '').trim();
  button.disabled = !!app.state?.search?.running || !hasQuery || !app.state?.current;
  button.dataset.searchRunButtonPass = 'v645-search-run-button-pass';
}

async function runSearch(app) {
  const request = buildSearchRunRequest(app);
  if (!beginSearchRun(app, request, { clearSearchJumpRuntimeState })) { syncSearchRunButton(app); renderResults(app); return; }
  renderResults(app);
  try {
