...
export function formatNovelMeta(app, novel, ratio, active) {
  ...
  const bookmarkCount = (app.state.bookmarks || []).filter(bm => bm && bm.novelId === novel.id).length;
  const flags = [];
...
export function createNovelItem(app, novel, deps = {}) {
  ...
  return createEl('div', { class:`novel-item${active ? ' active' : ''}${multi ? ' multi-file' : ''}${expanded ? ' episode-open' : ''}`, ...getLibraryDraggableAttrs(app, 'novel', { novelId:novel.id }, deps) }, [
    ...
    createEl('div', { class:'novel-meta', text: formatNovelMeta(app, novel, ratio, active) }),
...
export function appendNovelItem(app, container, novel, deps = {}) {
  ...
  if (novel.isMultiFile) container.append(createEpisodeList(app, novel, deps));
}
