...
const OWNER_STYLE_HREF = '/styles/owner.css?v=rebuild-v682';
...
export function ensureOwnerStylesLoaded(doc = document) {
  ...
  if (existing?.dataset?.ownerCssLoaded === '1') return Promise.resolve(true);
  ...
  ownerStyleLoadPromise = new Promise(resolve => {
    ...
    link.onload = () => {
      link.dataset.ownerCssLoaded = '1';
      ownerStyleLoadPromise = null;
      resolve(true);
    };
    link.onerror = () => {
      link.remove();
