...
검색 panel은 상태별 dataset을 사용하며, 모바일 idle/empty 상태에서 고정 68vh 최소 높이를 사용하지 않는다. modal과 결과 영역은 UI CSS 변수만 사용한다. 검색 실행, cache-only 기본값, 전체검색 권한, 결과 이동 좌표는 기존 계약을 유지한다.

<!-- v647-reader-search-modal-contract-pass -->
