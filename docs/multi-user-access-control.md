# v606 제한 사용자 cover 계약

표지 접근은 owner/all-library와 folder-limited 경로를 분리한다. 제한 경로는 folder ACL로 필터한 작품의 alias/work-key scope만 허용하며 library generation 변경 시 cache를 새로 만든다. 기준 marker는 `rebuild-v613`이다.

# v605 사용자 상태 ACL

폴더 제한과 접근 없음 계정은 사용자 상태를 읽을 때뿐 아니라 저장할 때도 ACL을 적용한다. 적용 대상은 favorites, bookmarks, recents, last opened, progress `lastRead/byNovel/readMeta/positions`, `novelUserTags`, device `collapsedFolders`다. 전체 라이브러리 계정은 catalog-free fast path를 유지한다.

## v601 사용자 태그 접근 경계

사용자 태그는 현재 로그인 사용자의 상태에만 저장된다. 다른 사용자의 태그 정의·적용값은 shelf response cache와 facet response cache key에 섞이지 않는다.

기준 버전: `rebuild-v605`

## v600 사용자 태그 격리

사용자 태그 정의와 작품 적용값은 인증된 사용자 상태 namespace에만 저장된다. owner 또는 다른 독서 계정의 태그를 합치지 않으며, 서재 shelf/tree cache key에도 사용자별 태그 상태를 포함한다. 태그 기능은 라이브러리 ACL을 확장하지 않고 이미 접근 가능한 작품에만 표시된다.
...
- 일반 사용자 namespace는 `data/user-data/<userId>/state.json`을 사용한다. legacy 통합 상태인 루트 `sync_data.json(.bak)`은 신규 `data/sync_data.json`이 없을 때만 읽어 비파괴 이관하며, 사용자별 namespace로 자동 분해하지는 않는다.
...
`appPermissions.metadataAccess`는 사용자별 기능 권한이며 기본값은 `false`다. owner는 항상 허용된다. 일반 사용자는 해당 값이 `true`일 때만 메타데이터 탭, `/metadata.html`, provider 및 작품 metadata 조회 API를 사용할 수 있다. 권한 회수는 계정의 `accessVersion`을 증가시켜 다음 access snapshot에서 UI를 숨기며 서버는 즉시 직접 요청을 거부한다.

메타데이터 변경 권한은 별도 조건을 가진다. `metadataAccess=true`이면서 전체 라이브러리 접근인 사용자 또는 owner만 수집·적용·삭제할 수 있다. 폴더 접근 사용자는 허용 폴더의 작품을 조회만 한다.

<!-- v588-metadata-access-control-pass -->

## v593 metadata 메뉴 표시와 로그아웃

- `metadataAccess`가 없는 사용자는 서재 HTML의 metadata 링크와 작품 action 버튼이 기본 `hidden` 상태이며 access snapshot 수신 후에도 숨김을 유지한다.
- 권한이 있는 사용자만 UI를 표시하되 직접 `/metadata.html` 및 API 접근은 서버에서 다시 검사한다.
- 서재·metadata page의 로그아웃은 동일한 `/api/logout` 계약을 사용한다.

<!-- v593-metadata-ui-access-pass -->
