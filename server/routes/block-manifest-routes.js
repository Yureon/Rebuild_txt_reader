...
function sendManifestError(res, err) {
  ...
  if (err && ['LIBRARY_COLD_BUILD_PENDING','LIBRARY_COLD_BUILD_BACKOFF','LIBRARY_COLD_BUILD_FAILED','LIBRARY_SYNC_COLD_BUILD_DISABLED'].includes(err.code)) {
    res.setHeader('Retry-After', String(Math.max(1, Number(err.retryAfterSeconds) || 1)));
    return res.status(503).json({ ok:false, error:'library_warming', message:'서재 저장소가 준비 중이거나 연결이 불안정합니다. 잠시 후 다시 시도해 주세요.', retryAfterSeconds:Math.max(1, Number(err.retryAfterSeconds) || 1), pass:err.pass || '' });
  }
  if (err && (err.code === 'CONTENT_WORKER_QUEUE_FULL' || err.code === 'CONTENT_WORKER_LARGE_FILE_FAILED' || Number(err.status || err.statusCode) === 503)) {
    res.setHeader('Retry-After', String(Math.max(1, Number(err.retryAfterSeconds) || 1)));
    return res.status(503).json({ ok:false, error:err.code === 'CONTENT_WORKER_QUEUE_FULL' ? 'content_worker_busy' : 'content_worker_failed', message:'대용량 TXT 처리 리소스가 포화되었거나 워커 처리가 실패했습니다. 잠시 후 다시 시도해 주세요.', pass:err.pass || '', size:Math.max(0, Number(err.size) || 0), maxQueuedTasks:Math.max(0, Number(err.maxQueuedTasks) || 0) });
  }
  ...
  console.error(err);
  return res.status(500).json({ error:'internal_server_error', message:'block manifest request failed' });
}
