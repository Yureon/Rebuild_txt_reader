...
} = require('../services/streaming-normalized-content-builder');
const { NORMALIZED_CONTENT_CACHE_PASS } = require('../services/normalized-content-cache');
...
async function run(payload = {}) {
  ...
  if (payload.storageMode === 'normalized-disk-cache') {
    const built = await buildStreamingNormalizedContentCache(payload);
    return {
      pass: CONTENT_ENTRY_WORKER_PASS,
      normalizedCachePass: NORMALIZED_CONTENT_CACHE_PASS,
      streamingBuildPass: STREAMING_NORMALIZED_CONTENT_BUILD_PASS,
      storage: 'disk',
      cacheKey: built.descriptor.cacheKey,
      ...
      sourceEncoding: built.sourceEncoding || '',
      sourceEncodingHeuristic: built.sourceEncodingHeuristic || '',
      sourceSampleBytes: Math.max(0, Number(built.sourceSampleBytes) || 0),
      indexBytes: Buffer.byteLength(JSON.stringify(built.chunkBounds || []), 'utf8'),
      fileReadMs: built.fileReadMs,
      fileReadCalls: built.fileReadCalls || 1
    };
  }
