const { ensurePlainObject, isSafeRecordKey } = require('./state-normalizer-core');
const { normalizeCustomCss: sanitizeCustomCss, MAX_CUSTOM_CSS_CHARS } = require('./custom-css-sanitizer');
...
const ALLOWED_VIEWER_PREF_KEYS = new Set([
  ...
  'tapDirection','tapScrollPercent','tapAnim','tapSpeed','swipeNav','swipeThreshold','timezone','timezoneOffset',
  'readerEpisodeBoundaryMode',
...
function normalizeSiteLanguageMap(input) {
  ...
  Object.keys(input).slice(0, 600).forEach((rawKey) => {
    ...
    if (isSafeRecordKey(key) && value) out[key] = value;
  });
  return out;
}
...
function sanitizeThemeBucket(input) {
  ...
  if (ensurePlainObject(input.themes)) {
    Object.keys(input.themes).slice(0, 20).forEach((themeId) => {
      ...
      const safeThemeId = String(themeId).trim().slice(0, 80) || 'default';
      if (isSafeRecordKey(safeThemeId)) themes[safeThemeId] = safeVars;
    });
  }
