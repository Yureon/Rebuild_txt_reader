...
const { loadJsonWithBackup, atomicWriteJsonSync } = require('../repositories/json-file-store');
...
function createSignupCodeService(options = {}) {
  ...
  function cloneDocument() {
    return normalizeSignupCodesDocument(JSON.parse(JSON.stringify(doc)));
  }
  ...
  function load() {
    ...
    if (loaded.source === 'backup') {
      try { writeJsonSync(signupCodesPath, doc); }
      catch (error) { if (logger && typeof logger.error === 'function') logger.error('signup code store backup recovery heal failed:', error.message); }
    }
    return doc;
  }
  ...
  function save(cb = () => {}) {
    try {
      writeJsonSync(signupCodesPath, doc);
      cb(null);
      return true;
    } catch (err) {
  ...
  function commitMutation(previous, out, cb = () => {}) {
    try {
      ...
      doc = previous;
      if (logger && typeof logger.error === 'function') logger.error('signup code mutation rollback:', err.message);
      cb(err);
      return null;
    }
  }
  ...
  function consumeCodeForRegistration(rawCode, createUser, cb = () => {}) {
    ...
    try {
      ...
      record.lastUsedBy = normalizeUsername(user && (user.username || user.id) || '');
      record.updatedAt = new Date().toISOString();
      let usageMetadataPersisted = true;
      try {
        saveOrThrow();
      } catch (error) {
        restoreRecordUsage(record, reserved);
        usageMetadataPersisted = false;
        if (logger && typeof logger.error === 'function') logger.error('signup code final usage metadata save failed after account commit:', error.message);
      }
      out = { ok: true, pass: TXT_READER_MULTI_REGISTER_PASS, code: publicCode(record), user, usageMetadataPersisted };
      cb(null, out);
      return out;
    } catch (err) {
      cb(err);
  ...
  function consumeCodeForRegistrationAsync(rawCode, createUser) {
    if (typeof createUser !== 'function') return Promise.reject(signupCodeError(500, '가입 처리기가 준비되지 않았습니다.', 'SIGNUP_CONSUMER_UNAVAILABLE'));
    const operation = async () => {
      ...
      let user;
      try {
        user = await createUser(publicCode(record));
      } catch (error) {
        restoreRecordUsage(record, previous);
        try { saveOrThrow(); } catch (rollbackError) {
          if (logger && typeof logger.error === 'function') logger.error('signup code consume rollback failed:', rollbackError.message);
      ...
      return { ok:true, pass:TXT_READER_MULTI_REGISTER_PASS, code:publicCode(record), user, usageMetadataPersisted };
    };
    const task = registrationQueue.catch(() => {}).then(operation);
    registrationQueue = task;
    return task;
  }
  ...
  return {
    ...
    consumeCodeForRegistrationAsync,
