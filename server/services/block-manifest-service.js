...
const FOLDER_MANIFEST_DISK_SCHEMA = 4;
const EPISODE_MANIFEST_DISK_SCHEMA = 3;
...
function createBlockManifestService(options = {}) {
  ...
  function resolveSingleFileNovel(novelId) {
  ...
  function resolveEpisode(novelId, episodeId) {
    const library = libraryService.getLibraryCached();
    const novel = library.find(n => n && n.id === novelId);
  ...
  async function getSingleManifest(novelId, query = {}, requestOptions = {}) {
    const base = resolveNovel(novelId);
    ...
    const resolved = resolveSingleFileNovel(novelId);
  ...
  async function getEpisodeManifest(novelId, episodeId, query = {}, requestOptions = {}) {
    ...
    const resolved = resolveEpisode(novelId, episodeId);
    if (resolved.status) return resolved;
    const preprocessOptions = contentService.parsePreprocessOptionsFromQuery(query);
  ...
  async function buildMultiFileManifestFresh(novel, episodes, preprocessOptions, preprocessSignature, structuralKey, folderOptions, signal = null) {
    ...
    assertNotAborted(signal);
    const folderSignature = resolveFolderManifestSignature(novel, episodes, preprocessSignature, structuralKey, folderOptions);
    const cachedFolderManifest = readFolderManifestFromDisk(folderSignature);
    ...
    writeFolderManifestToDisk(folderSignature, manifest);
  ...
  async function buildManifestWithEpisodeDiskCache(resolved, preprocessOptions, episodeIndex = -1, cacheOptions = {}) {
    ...
    assertNotAborted(signal);
    const preprocessSignature = contentService.serializePreprocessOptions(preprocessOptions);
    const signature = buildEpisodeManifestSignature(resolved, preprocessSignature, episodeIndex);
    const writeDisk = cacheOptions.writeDisk !== false;
    const cached = readEpisodeManifestFromDisk(signature);
    if (cached) {
    ...
    if (inflight) {
      ...
      if (writeDisk) writeEpisodeManifestToDisk(signature, joinedManifest);
    ...
    if (writeDisk) writeEpisodeManifestToDisk(signature, manifest);
    else manifestCacheMetrics.episodeManifestDiskWriteSkippedForFolderBuild += 1;
    return manifest;
  }
  ...
  async function buildManifest(resolved, preprocessOptions, episodeDiskSignature = null, requestOptions = {}) {
    ...
    for (let chunk = 1; chunk <= totalChunks; chunk += 1) {
      assertNotAborted(signal);
      const range = contentService.getChunkByLine(entry, chunk);
...
module.exports = { createBlockManifestService, splitContentBlocks, BLOCK_MANIFEST_CACHE_STATUS_PASS, BLOCK_MANIFEST_CACHE_STRATEGY_PASS, BLOCK_MANIFEST_DISK_CACHE_PASS, BLOCK_MANIFEST_EPISODE_DISK_CACHE_PASS, BLOCK_MANIFEST_EPISODE_DISK_CACHE_V469_PASS, BLOCK_MANIFEST_STABLE_ETAG_PASS, BLOCK_MANIFEST_FOLDER_BUILD_THROTTLE_PASS, FOLDER_BLOCK_MANIFEST_WINDOW_PASS };
