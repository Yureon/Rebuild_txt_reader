...
function validateSharedState(input) {
  ...
  if (input.userTags && (!Array.isArray(input.userTags) || input.userTags.length > 100)) {
    return { ok: false, error: 'userTags invalid' };
  }

  if (input.novelUserTags && (!ensurePlainObject(input.novelUserTags) || Object.keys(input.novelUserTags).length > 5000)) {
    return { ok: false, error: 'novelUserTags invalid' };
  }

  if (input.novelUserTags && Object.values(input.novelUserTags).some(tags => !Array.isArray(tags) || tags.length > 20)) {
    return { ok: false, error: 'novelUserTags entries invalid' };
  }
  ...
  if (input.syncPolicy && Array.isArray(input.syncPolicy.devices) && input.syncPolicy.devices.length > MAX_DEVICE_PROFILES) {
