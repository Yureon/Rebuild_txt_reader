...
async function accessStatusAsync(targetPath, mode) {
  const out = { path:targetPath, exists:false, ok:false, error:'' };
  try {
    await fs.promises.access(targetPath, mode);
    out.exists = true;
    out.ok = true;
  } catch (error) {
    if (error && error.code !== 'ENOENT') out.exists = true;
    out.error = error && error.message || String(error);
  }
  return out;
}

async function directoryWritableStatusAsync(dirPath) {
  try { await fs.promises.mkdir(dirPath, { recursive:true }); } catch {}
  return accessStatusAsync(dirPath, fs.constants.R_OK | fs.constants.W_OK);
}

async function fileParentWritableStatusAsync(filePath) {
  if (!filePath) return { path:'', exists:false, ok:false, error:'path unavailable' };
  return directoryWritableStatusAsync(path.dirname(filePath));
}
...
function createAdminDiagnosticsService(options = {}) {
  const { paths, env, sessionStore, accountService, auditLogService, libraryService, contentService, blockManifestService, diskCacheJanitorService, libraryContentFingerprintService, libraryVariantPreferenceService, metadataService } = options;
  if (!paths || !env) throw new Error('createAdminDiagnosticsService requires paths/env');

  async function buildDiagnostics(req) {
    if (auditLogService && typeof auditLogService.flush === 'function') await auditLogService.flush();
    const libraryPath = env.LIBRARY_PATH || '';
    let libraryNovelCount = null;
    let libraryError = '';
    if (libraryService && (typeof libraryService.getLibraryCachedAsync === 'function' || typeof libraryService.getLibraryCached === 'function')) {
      try {
    ...
    const forwardedProtoTrusted = String(runtime.deploymentMode || 'direct').trim().toLowerCase() !== 'direct' && forwardedProto === 'https';
    const effectiveSecure = requestSecure || requestProtocol.toLowerCase() === 'https' || forwardedProtoTrusted || cloudflareVisitorHttpsTrusted;
    ...
    const [dataDir, accountsStore, signupCodesStore, userDataDir, fontDir, sessionStoreStatus, auditLog, libraryPathStatus] = await Promise.all([
      directoryWritableStatusAsync(paths.DATA_DIR),
      fileParentWritableStatusAsync(paths.ACCOUNTS_PATH),
      paths.SIGNUP_CODES_PATH ? fileParentWritableStatusAsync(paths.SIGNUP_CODES_PATH) : Promise.resolve({ ok:false, error:'signup code path unavailable' }),
      directoryWritableStatusAsync(paths.USER_DATA_DIR),
      directoryWritableStatusAsync(paths.FONT_DIR),
      fileParentWritableStatusAsync(paths.SESSION_STORE_PATH),
      auditLogService && typeof auditLogService.getStatusAsync === 'function'
        ? auditLogService.getStatusAsync()
        : auditLogService && typeof auditLogService.getStatus === 'function'
          ? Promise.resolve(auditLogService.getStatus())
          : Promise.resolve({ ok:false, error:'audit log service unavailable' }),
      accessStatusAsync(libraryPath, fs.constants.R_OK)
    ]);
    const storage = {
      dataDir,
      accountsStore,
    ...
    const diskCacheDiagnostics = diskCacheJanitorService && typeof diskCacheJanitorService.getStatusAsync === 'function'
      ? await diskCacheJanitorService.getStatusAsync()
      : safeDiskCacheJanitorStatus(diskCacheJanitorService);
    ...
    return {
      ...
      ioDiagnostics: (() => {
        const ioDiagnostics = {
          ...
          diskCacheJanitor: diskCacheDiagnostics,
          libraryContentFingerprint: libraryContentFingerprintService && typeof libraryContentFingerprintService.getStatus === 'function'
            ? libraryContentFingerprintService.getStatus()
            : { available:false },
          libraryVariantPreferences: libraryVariantPreferenceService && typeof libraryVariantPreferenceService.list === 'function'
            ? { available:true, revision:libraryVariantPreferenceService.getRevision?.() || 0, ...libraryVariantPreferenceService.list() }
            : { available:false },
          metadataStorage: metadataService && typeof metadataService.metadataStorageStats === 'function'
            ? { available:true, ...metadataService.metadataStorageStats() }
            : { available:false }
        };
