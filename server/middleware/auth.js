...
const READER_ENTRY_PATHS = new Set(['/', '/index.html', '/site.html', '/mobile.html']);
  ...
  return function requireSameOrigin(req, res, next) {
    ...
    if (origin) {
      if (!allowedOrigins.includes(origin)) {
        return res.status(403).json({ error: 'origin blocked', origin, allowedOrigins });
      }
      return next();
    }
    ...
    if (referer) {
      ...
      if (!refOrigin || !allowedOrigins.includes(refOrigin)) {
        return res.status(403).json({ error: 'referer blocked', referer: refOrigin, allowedOrigins });
      }
      return next();
    }
    ...
    if (unsafe && requireStrictOrigin) {
      ...
      return res.status(403).json({ error: 'origin required', allowedOrigins });
    }
